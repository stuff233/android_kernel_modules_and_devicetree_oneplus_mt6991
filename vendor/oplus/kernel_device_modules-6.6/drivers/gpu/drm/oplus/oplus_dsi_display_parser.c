...
int oplus_dsi_panel_parser_base(struct device_node *node, void *ext_param_dev, void *ctx_dev)
{
	...
	rc = utils->get_int(node, "oplus,lfcc-dbv-cfg", &ctx->lfcc_dbv_cfg);
	rc = utils->get_bool(node, "oplus,ofp-need-aod-off-insert-black-frame", &ctx->need_aod_off_insert_black_frame);
...
int oplus_dsi_panel_parser_timing_base(struct device_node *node, void *ext_param_dev, void *ctx_dev)
{
	...
	rc = utils->get_int(node, "oplus,ofp-aod-off-setbacklight-delay", &ext_param->oplus_ofp_aod_off_setbacklight_delay);

/**********************************************VIDLE*************************************************************************************/
	if(utils->get_int(node, "oplus,is-support-vidle", &ext_param->is_support_vidle) == 0) {
		rc = of_property_read_u32(node, "oplus,dsi-panel-framerate", &framerate);
		OPLUS_DSI_INFO("framerate = %d\n", framerate);
		if (of_property_read_u32(node, "oplus,adfr-timing-type", &adfr_timing_type)) {
			adfr_timing_type = 0;
			OPLUS_DSI_WARN("adfr_timing_type parse failed, default set to %d\n", adfr_timing_type);
		} else {
			OPLUS_DSI_INFO("adfr_timing_type = %d\n", adfr_timing_type);
		}
		rc = utils->get_int(node, "oplus,vidle-postsz", &ext_param->oplus_vidle_postsz);
		if (rc == 0) {
			if ((adfr_timing_type == 0) && (framerate == 60)) {
				cell_index = 0;
			} else if ((adfr_timing_type == 0) && (framerate == 90)) {
				cell_index = 1;
			} else if ((adfr_timing_type == 0) && (framerate == 120)) {
				cell_index = 2;
			} else if ((adfr_timing_type == 2) && (framerate == 120)) {
				/* adfr mode don't enable, use default 500us */
			}
		}
		/********************************************** /
		/*g_vidle_support[0][*] = is_support_vidle     */
		/*g_vidle_support[1][*]  =  oplus_vidle_postsz */
		/***************** *****************************/
		g_vidle_support[0][cell_index]  = ext_param->is_support_vidle;
		g_vidle_support[1][cell_index] = ext_param->oplus_vidle_postsz;

		if (g_vidle_support[1][cell_index] == 0) {
			g_vidle_support[1][cell_index] = 500; /*use default 500us */
		}
		OPLUS_DSI_INFO("oplus_dsi_panel_parser_timing_base vidle=%d,framerate=%d vidle_postsz[%d]=%d\n",
		g_vidle_support[0][cell_index], framerate, cell_index, g_vidle_support[1][cell_index]);
	}
/**********************************************VIDLE**************************************************************************************/
	return rc;
}
