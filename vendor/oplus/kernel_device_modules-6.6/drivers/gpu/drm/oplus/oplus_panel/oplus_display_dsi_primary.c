...
extern char regs[AC247_GAMMA_COMPENSATION_READ_LENGTH];
...
static int panel_lhbm_pressed_icon_grayscale_update(void *para_list, unsigned int bl_level)
{
	...
	unsigned char tx_buf_1[5] = {0xB8, 0x00, 0x00, 0x0F, 0xFF};
	int rc = 0;
	...
	if (!strcmp("ac262_p_7_a0014_dsi_cmd_panel", panel_name) || !strcmp("panel_aa599_p_7_a0014_dsi_cmd", panel_name)) {
		if (!oplus_panel_pwm_onepulse_is_enabled()) {
			if ((bl_level > 0x0000) && (bl_level <= 0x0482)) {
				...
				tx_buf[1] = (2048 * (bl_level - 1154) / (1869 - 1154)) >> 8;
				tx_buf[2] = (2048 * (bl_level - 1154) / (1869 - 1154)) & 0xFF;
				tx_buf[3] = 4020 >> 8;
				tx_buf[4] = 4020 & 0xFF;
				need_to_update_grayscale = true;
			} else if ((bl_level > 0x074D) && (bl_level <= 0x0DBB)) {
				tx_buf[1] = ((2362 - 2048) * (bl_level - 1869) / (3515 - 1869) + 2048) >> 8;
				tx_buf[2] = ((2362 - 2048) * (bl_level - 1869) / (3515 - 1869) + 2048) & 0xFF;
				tx_buf[3] = 4020 >> 8;
				tx_buf[4] = 4020 & 0xFF;
				need_to_update_grayscale = true;
			} else if ((bl_level > 0x0DBB) && (bl_level <= 0x0FFE)) {
				tx_buf[1] = ((2048 - 2362) * (bl_level - 3515) / (4095 - 3515) + 2362) >> 8;
				tx_buf[2] = ((2048 - 2362) * (bl_level - 3515) / (4095 - 3515) + 2362) & 0xFF;
				tx_buf[3] = ((bl_level - 3515) / (4095 - 3515) * (4095 - 4020) + 4020) >> 8;
				tx_buf[4] = ((bl_level - 3515) / (4095 - 3515) * (4095 - 4020) + 4020) & 0xFF;
				need_to_update_grayscale = true;
			}
		} else {
			if ((bl_level > 0x0000) && (bl_level <= 0x0482)) {
...
static int oplus_display_panel_set_demura_bl(void *dsi, void *handle, int bl_lvl, enum dsi_cmd_func cmd_state)
{
	...
	if (!strcmp("panel_ac178_p_7_a0014_dsi_cmd", panel_name) || !strcmp("panel_ac266_p_7_a0014_dsi_cmd", panel_name)) {
...
static int oplus_display_panel_set_pulse_bl(void *dsi, void *handle, int bl_lvl, enum dsi_cmd_func cmd_state)
{
	int ret = 0;
	...
	if (bl_lvl > last_backlight) {
		if (bl_lvl > oplus_display0_params->lfcc_dbv_cfg && last_backlight <= oplus_display0_params->lfcc_dbv_cfg) {
			ret = oplus_dsi_panel_send_cmd(dsi, DSI_CMD_ENABLE_LFCC, handle, cmd_state);
		}
	} else if (bl_lvl < last_backlight) {
		if (bl_lvl <= oplus_display0_params->lfcc_dbv_cfg && last_backlight > oplus_display0_params->lfcc_dbv_cfg) {
			ret = oplus_dsi_panel_send_cmd(dsi, DSI_CMD_ENABLE_LFCC, handle, cmd_state);
		}
	}
...
static int oplus_panel_set_backlight_cmdq(void *dsi, dcs_write_gce_pack cb, void *handle, unsigned int level)
{
...
#ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT
	if (strcmp("panel_ac178_p_7_a0014_dsi_cmd", panel_name) == 0 || strcmp("panel_ac266_p_7_a0014_dsi_cmd", panel_name) == 0) {
	...
	} else if (strcmp("ac262_p_7_a0014_dsi_cmd_panel", panel_name) == 0 || strcmp("panel_aa599_p_7_a0014_dsi_cmd", panel_name) == 0) {
		if (oplus_ofp_is_supported()) {
			if (lcm_all_cmd_table[DSI_CMD_LHBM_PRESSED_ICON_GRAYSCALE].cmd_lines > 0) {
				if (panel_lhbm_pressed_icon_grayscale_update(
						lcm_all_cmd_table[DSI_CMD_LHBM_PRESSED_ICON_GRAYSCALE].para_table[2].para_list, level) == 1) {
					oplus_dsi_panel_send_cmd(dsi, DSI_CMD_LHBM_PRESSED_ICON_GRAYSCALE,
							handle, cmd_state);
	...
	if (strcmp("panel_ac247_p_7_a0014_dsi_cmd", panel_name) == 0) {
		oplus_display_panel_set_pulse_bl(dsi, handle, level, cmd_state);
	}
...
static void AC262_set_gamma_compensation(void *dsi_drv, void *handle)
{
	u32 index = 0;
	u32 seed_gain = 0;
	...
	OPLUS_DSI_DEBUG("temp_seed_mode=%d\n", temp_seed_cmd);
	...
	OFP_INFO("AC262_Gamma_Compensation End\n");
}

static void AC247_set_gamma_compensation(void *dsi_drv, void *handle)
{
	u32 index = 0;
	char r_reg1 = 0;
	char r_reg2 = 0;
	...
	OPLUS_DSI_DEBUG("temp_seed_mode=%d\n", temp_seed_cmd);
	if (temp_seed_cmd == DSI_CMD_SET_SEED_NATURAL) {
		r_reg1 = 993 * regs[0]/1000;
		r_reg2 = 993 * regs[1]/1000;
		g_reg1 = 993 * regs[2]/1000;
		g_reg2 = 993 * regs[3]/1000;
		b_reg1 = 993 * regs[4]/1000;
		b_reg2 = 993 * regs[5]/1000;
		OPLUS_DSI_DEBUG("DSI_CMD_SET_SEED_NATURAL mode=%d\n", temp_seed_cmd);
	} else if (temp_seed_cmd == DSI_CMD_SET_SEED_VIVID) {
		r_reg1 = 982 * regs[0]/1000;
		r_reg2 = 982 * regs[1]/1000;
		g_reg1 = 982 * regs[2]/1000;
		g_reg2 = 982 * regs[3]/1000;
		b_reg1 = 982 * regs[4]/1000;
		b_reg2 = 982 * regs[5]/1000;
		OPLUS_DSI_DEBUG("DSI_CMD_SET_SEED_VIVID mode=%d\n", temp_seed_cmd);
	}
	OFP_DEBUG("compensation regs1=[%02X %02X], regs2=[%02X %02X], regs3=[%02X %02X]\n",
			r_reg1, r_reg2, g_reg1, g_reg2, b_reg1, b_reg2);
	OFP_DEBUG("regs[0 1] = %x,%x, regs[2 3] = %x,%x regs[4 5] = %x,%x\n",
			regs[0], regs[1], regs[2], regs[3], regs[4], regs[5]);

	for (index = 0; index < ctx->mode_num; index++) {
		cmd_table = lcm_cur_cmd_table(index)[DSI_CMD_GAMMA_COMPENSATION].para_table;
		if ((lcm_cur_cmd_table(index)[DSI_CMD_GAMMA_COMPENSATION].cmd_lines >= AC247_GAMMA_COMPENSATION_CMD_INDEX + 1) && \
				(cmd_table[AC247_GAMMA_COMPENSATION_CMD_INDEX].count >= (AC247_GAMMA_COMPENSATION_REG_INDEX6 + 1))) {
			cmd_table[AC247_GAMMA_COMPENSATION_CMD_INDEX].para_list[AC247_GAMMA_COMPENSATION_REG_INDEX1 + 1] = r_reg1;
			cmd_table[AC247_GAMMA_COMPENSATION_CMD_INDEX].para_list[AC247_GAMMA_COMPENSATION_REG_INDEX2 + 1] = r_reg2;
			cmd_table[AC247_GAMMA_COMPENSATION_CMD_INDEX].para_list[AC247_GAMMA_COMPENSATION_REG_INDEX3 + 1] = g_reg1;
			cmd_table[AC247_GAMMA_COMPENSATION_CMD_INDEX].para_list[AC247_GAMMA_COMPENSATION_REG_INDEX4 + 1] = g_reg2;
			cmd_table[AC247_GAMMA_COMPENSATION_CMD_INDEX].para_list[AC247_GAMMA_COMPENSATION_REG_INDEX5 + 1] = b_reg1;
			cmd_table[AC247_GAMMA_COMPENSATION_CMD_INDEX].para_list[AC247_GAMMA_COMPENSATION_REG_INDEX6 + 1] = b_reg2;
		} else {
			OFP_INFO("Invalid mode[%d]-dsi_cmd: DSI_CMD_GAMMA_COMPENSATION\n", index);
		}
	}
	if(temp_seed_cmd == DSI_CMD_SET_SEED_NATURAL || temp_seed_cmd == DSI_CMD_SET_SEED_VIVID) {
		oplus_dsi_panel_send_cmd(dsi_drv, DSI_CMD_GAMMA_COMPENSATION, handle, DSI_CMD_FUNC_DEFAULT);
	}
	OFP_DEBUG("AC247_Gamma_Compensation End\n");
}
...
static int oplus_ofp_set_lhbm_pressed_icon(struct drm_panel *panel, void *dsi_drv, dcs_write_gce_pack cb, void *handle, uint64_t lhbm_pressed_icon_on)
{
	...
	if (lhbm_pressed_icon_on) {
		if (strcmp("panel_ac178_p_7_a0014_dsi_cmd", panel_name) == 0 || strcmp("panel_ac266_p_7_a0014_dsi_cmd", panel_name) == 0) {
		...
		} else if (strcmp("panel_ac247_p_7_a0014_dsi_cmd", panel_name) == 0) {
			AC247_set_gamma_compensation(dsi_drv, handle);
			if (oplus_display_brightness > 0x0DBB) {
				cmd_set_id = DSI_CMD_LHBM_PRESSED_ICON_ON;
			} else {
				cmd_set_id = DSI_CMD_LHBM_PRESSED_ICON_ON_PWM;
			...
			AC262_set_gamma_compensation(dsi_drv, handle);
			if (oplus_display_brightness > LHBM_BACKLIGHT_THRESHOLD) {
				cmd_set_id = DSI_CMD_LHBM_PRESSED_ICON_ON;
			} else {
				cmd_set_id = DSI_CMD_LHBM_PRESSED_ICON_ON_PWM;
	...
	if (!lhbm_pressed_icon_on) {
		if (!oplus_display_brightness || !oplus_ofp_backlight_filter(crtc, handle, oplus_display_brightness)) {
...
static ktime_t panel_updata_doze_disable_time(void) {
	return aod_off_time;
}
...
static int panel_doze_disable(struct drm_panel *panel, void *dsi, dcs_write_gce_pack cb, void *handle)
{
	...
	} else if (oplus_display0_params->need_aod_off_insert_black_frame) {
		if (handle) {
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.oplus_get_doze_disable_time = panel_updata_doze_disable_time,
