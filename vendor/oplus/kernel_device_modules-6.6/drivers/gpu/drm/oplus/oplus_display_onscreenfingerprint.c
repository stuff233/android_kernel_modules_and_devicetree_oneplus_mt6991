...
bool oplus_ofp_video_mode_30hz_aod_is_enabled(void)
{
        ...
        if (p_oplus_ofp_params->video_mode_30hz_aod_accelerate) {
                OFP_DEBUG("video mode 30hz aod accelerate solution is set\n");
                return false;
        } else {
...
bool oplus_ofp_video_mode_30hz_aod_accelerate_is_enabled(void)
{
        struct oplus_ofp_params *p_oplus_ofp_params = oplus_ofp_get_params();
        ...
        if (!p_oplus_ofp_params->video_mode_30hz_aod_accelerate) {
                OFP_DEBUG("video mode 30hz aod accelerate solution not set\n");
                return false;
        } else {
                return (bool)(OPLUS_OFP_GET_VIDEO_MODE_30HZ_AOD_CONFIG(p_oplus_ofp_params->fp_type));
        }
...
EXPORT_SYMBOL(oplus_ofp_local_hbm_unlocking_acceleration_is_enabled);
...
static int oplus_ofp_set_panel_hbm(void *drm_crtc, bool hbm_en)
{
	bool doze_en = false;
	struct drm_crtc *crtc = drm_crtc;
	...
	if (oplus_ofp_video_mode_30hz_aod_accelerate_is_enabled() && oplus_ofp_get_aod_state()) {
		OFP_INFO("should not set panel hbm if panel is in video mode aod state\n");
		return 0;
	}
	...
	if (!crtc_state) {
		OFP_ERR("Invalid crtc_state param\n");
		return -EINVAL;
	}
	...
	OFP_INFO("flush hbm cmd\n");
	oplus_ofp_set_hbm_state(hbm_en);
	oplus_ofp_lhbm_resend_uiready(mtk_crtc);
...
/* due to the involvement of the doze/doze suspend power mode scenario, add new function to send lhbm commands */
static int oplus_ofp_set_panel_lhbm(void *drm_crtc, bool lhbm_en)
{
	bool doze_en = false;
	...
	if (!(oplus_ofp_local_hbm_is_enabled() && oplus_ofp_local_hbm_unlocking_acceleration_is_enabled())) {
		OFP_DEBUG("no need to set panel lhbm\n");
		return 0;
	}
	...
	if (drm_crtc_index(crtc) != 0) {
		OFP_DEBUG("not in dsi mode, should not set panel lhbm\n");
		return 0;
	}
	...
	if (oplus_ofp_get_hbm_state() == lhbm_en) {
		OFP_DEBUG("already in hbm state %d\n", lhbm_en);
		return 0;
	}

	OPLUS_OFP_TRACE_BEGIN("oplus_ofp_set_panel_lhbm");

	if (!crtc_state->prop_val[CRTC_PROP_DOZE_ACTIVE]) {
		OFP_INFO("not in doze mode\n");
	}

	OPLUS_OFP_TRACE_BEGIN("DDP_MUTEX_LOCK");
	DDP_MUTEX_LOCK(&mtk_crtc->lock, __func__, __LINE__);
	OPLUS_OFP_TRACE_END("DDP_MUTEX_LOCK");

	if (oplus_ofp_get_hbm_state() == lhbm_en) {
		OFP_INFO("already in hbm state %d\n", lhbm_en);
		DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
		OPLUS_OFP_TRACE_END("oplus_ofp_set_panel_lhbm");
		return 0;
	}

	output_comp = mtk_ddp_comp_request_output(mtk_crtc);
	if (unlikely(!output_comp)) {
		OFP_ERR("Invalid output_comp param\n");
		DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
		OPLUS_OFP_TRACE_END("oplus_ofp_set_panel_lhbm");
		return -ENODEV;
	}
	...
	if (!mtk_crtc->enabled) {
		...
		if (mtk_crtc_with_event_loop(crtc) && (mtk_crtc_is_frame_trigger_mode(crtc))) {
	...
	/* send LCM CMD */
	OPLUS_OFP_TRACE_BEGIN("prepare_to_send_lhbm_cmd");
	OFP_INFO("prepare to send lhm cmd\n");
	is_frame_mode = mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base);
	if (is_frame_mode || !mtk_crtc->gce_obj.client[CLIENT_DSI_CFG]) {
		cmdq_handle = cmdq_pkt_create(mtk_crtc->gce_obj.client[CLIENT_CFG]);
	} else {
	...
	if (!cmdq_handle) {
		DDPPR_ERR("%s:%d NULL cmdq handle\n", __func__, __LINE__);
		DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
		OPLUS_OFP_TRACE_END("prepare_to_send_lhbm_cmd");
		OPLUS_OFP_TRACE_END("oplus_ofp_set_panel_lhbm");
		return -EINVAL;
	}
	if (is_frame_mode) {
	...
	if (oplus_ofp_get_aod_state()) {
		OFP_INFO("send aod off cmd before lhbm on because panel is still in aod mode\n");
		oplus_ofp_aod_off_status_handle(mtk_crtc);

		OPLUS_OFP_TRACE_BEGIN("DSI_SET_DOZE");
		if (output_comp->funcs && output_comp->funcs->io_cmd) {
			output_comp->funcs->io_cmd(output_comp, cmdq_handle, DSI_SET_DOZE, &doze_en);
			OFP_INFO("DSI_SET_DOZE:%d\n", doze_en);
		}
		OPLUS_OFP_TRACE_END("DSI_SET_DOZE");
	}

	/* delay before lhbm cmd */
	oplus_ofp_hbm_wait_handle(crtc, cmdq_handle, true, lhbm_en);

	/* send lhbm pressed icon cmd */
	OPLUS_OFP_TRACE_BEGIN("OPLUS_OFP_SET_LHBM_PRESSED_ICON");
	if (output_comp->funcs && output_comp->funcs->io_cmd) {
		output_comp->funcs->io_cmd(output_comp, cmdq_handle, OPLUS_OFP_SET_LHBM_PRESSED_ICON, &lhbm_en);
		OFP_INFO("OPLUS_OFP_SET_LHBM_PRESSED_ICON:%d\n", lhbm_en);
	}
	OPLUS_OFP_TRACE_END("OPLUS_OFP_SET_LHBM_PRESSED_ICON");

	/* delay after lhbm cmd */
	oplus_ofp_hbm_wait_handle(crtc, cmdq_handle, false, lhbm_en);

	OPLUS_OFP_TRACE_BEGIN("flush_lhbm_cmd");
	if (is_frame_mode) {
	...
	OFP_INFO("flush lhbm cmd\n");
	oplus_ofp_set_hbm_state(lhbm_en);
	oplus_ofp_lhbm_resend_uiready(mtk_crtc);
	cmdq_pkt_destroy(cmdq_handle);
	OPLUS_OFP_TRACE_END("flush_lhbm_cmd");

	if (!lhbm_en && p_oplus_ofp_params->aod_unlocking) {
		p_oplus_ofp_params->aod_unlocking = false;
		OFP_INFO("oplus_ofp_aod_unlocking:%d\n", p_oplus_ofp_params->aod_unlocking);
		OPLUS_OFP_TRACE_INT("oplus_ofp_aod_unlocking|%d", p_oplus_ofp_params->aod_unlocking);
	}
...
*/
int oplus_ofp_lhbm_handle(void *drm_crtc, bool need_lock)
{
	int rc = 0;
	struct drm_crtc *crtc = drm_crtc;
	struct mtk_drm_crtc *mtk_crtc = NULL;
	...
	if (!mtk_crtc_is_frame_trigger_mode(crtc)) {
		OFP_DEBUG("crtc is not frame trigger mode\n");
		return 0;
	}
	...
	OFP_DEBUG("fp_press:%d,need_lock:%d,bl_level:%u\n",
				p_oplus_ofp_params->fp_press, need_lock, oplus_display_brightness);

	if (p_oplus_ofp_params->fp_press && oplus_display_brightness) {
		if (need_lock) {
			rc = oplus_ofp_set_panel_lhbm(crtc, true);
		} else {
			rc = oplus_ofp_set_panel_hbm(crtc, true);
		}
		if (rc) {
			OFP_ERR("failed to set panel lhbm on\n");
		}
	} else if (!p_oplus_ofp_params->fp_press || !oplus_display_brightness) {
		if (need_lock) {
			rc = oplus_ofp_set_panel_lhbm(crtc, false);
		} else {
			rc = oplus_ofp_set_panel_hbm(crtc, false);
		}
		if (rc) {
			OFP_ERR("failed to set panel lhbm off\n");
		}
	}
...
int oplus_ofp_notify_uiready(void *mtk_drm_crtc)
{
	...
	} else if (p_oplus_ofp_params->aod_unlocking == true) {
...
*/
int oplus_ofp_lhbm_resend_uiready(void *mtk_drm_crtc)
{
	ktime_t delta_time = 0;
	unsigned int refresh_rate = 0;
	unsigned int delay_ms = 0;
	...
	if (p_oplus_ofp_params->aod_unlocking) {
		...
		if (ktime_sub(ms_to_ktime(34), delta_time) > ms_to_ktime(9)) {
...
bool oplus_ofp_backlight_filter(void *drm_crtc, void *cmdq_pkt, unsigned int bl_level)
{
	...
	if (oplus_ofp_get_hbm_state()) {
				...
				if (!mtk_crtc) {
					...
					if (!(comp && comp->funcs && comp->funcs->io_cmd)) {
						...
						if (!mtk_crtc->enabled) {
							...
							/* send lhbm pressed icon cmd */
							OPLUS_OFP_TRACE_BEGIN("OPLUS_OFP_SET_LHBM_PRESSED_ICON");
							comp->funcs->io_cmd(comp, cmdq_handle, OPLUS_OFP_SET_LHBM_PRESSED_ICON, &lhbm_en);
							OFP_INFO("OPLUS_OFP_SET_LHBM_PRESSED_ICON:%d\n", lhbm_en);
							OPLUS_OFP_TRACE_END("OPLUS_OFP_SET_LHBM_PRESSED_ICON");
						}
					}
		...
		need_filter_backlight = true;
	} else if (oplus_ofp_get_aod_state() && !oplus_ofp_video_mode_30hz_aod_accelerate_is_enabled()) {
...
int oplus_ofp_aod_off_status_handle(void *mtk_drm_crtc)
{
	...
	if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
			&& mtk_crtc->panel_ext->params->oplus_ofp_aod_off_setbacklight_delay > 0) {
		if (p_oplus_ofp_params->aod_unlocking) {
...
int oplus_ofp_video_mode_aod_handle(void *drm_crtc, void *mtk_panel_ext, void *drm_panel, void *mtk_dsi, void *dcs_write_gce, void *cmdq_handle)
{
	...
	if (!oplus_ofp_get_aod_state() && (refresh_rate == 30)) {
		...
		/* aod on */
		if (ext && ext->funcs && ext->funcs->oplus_doze_enable) {
			OFP_INFO("debug for oplus_doze_enable\n");
			ext->funcs->oplus_doze_enable(drm_panel, mtk_dsi, dcs_write_gce, NULL);
		}
...
int oplus_ofp_aod_off_backlight_recovery(void *drm_crtc, void *mtk_crtc_state)
{
	...
	*/
	if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
		...
		OFP_DEBUG("frame_count:%u\n", frame_count);

		aod_off_setbacklight_delay = mtk_crtc->panel_ext->params->oplus_ofp_aod_off_setbacklight_delay;
		if (frame_count == aod_off_setbacklight_delay) {
			...
			if (atomic_read(&mtk_crtc->singal_for_mode_switch)) {
				DDPINFO("recovery backlight Wait event from mode_switch\n");
			...
			OFP_INFO("recovery backlight level = %d after %d frame delay\n", oplus_display_brightness, aod_off_setbacklight_delay);

			OPLUS_OFP_TRACE_BEGIN("flush recovery backlight");
			if (is_frame_mode) {
				cmdq_pkt_set_event(cmdq_handle,
					mtk_crtc->gce_obj.event[EVENT_CABC_EOF]);
				cmdq_pkt_set_event(cmdq_handle,
			...
			cmdq_pkt_flush(cmdq_handle);
			cmdq_pkt_destroy(cmdq_handle);
			OFP_DEBUG("flush recovery backlight\n");
			OPLUS_OFP_TRACE_END("flush recovery backlight");
		} else if (frame_count < aod_off_setbacklight_delay) {
			OFP_DEBUG("aod_off_setbacklight_delay:%u,frame_count:%u,need_to_filter_backlight_after_aod_off:%d\n",
...
int oplus_ofp_notify_fp_press(void *buf)
{
	...
	oplus_ofp_lhbm_handle(crtc, true);
