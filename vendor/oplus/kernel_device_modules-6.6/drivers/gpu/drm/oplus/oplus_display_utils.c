...
char regs[AC247_GAMMA_COMPENSATION_READ_LENGTH] = {0};
EXPORT_SYMBOL(regs);
static int oplus_panel_ac247_gamma_compensation(void *dsi)
{
	u32 retry_count = 0;
	u32 index = 0;
	u32 cnt = 0;
	char print_buf[OPLUS_DSI_CMD_PRINT_BUF_SIZE] = {0};
	char regs_last[AC247_GAMMA_COMPENSATION_READ_LENGTH] = {0};
	const char regs_base[AC247_GAMMA_COMPENSATION_READ_LENGTH] = {0};
	if (dsi == NULL) {
	        OPLUS_DSI_ERR("Invalid DSI pointer\n");
		return -EINVAL;
	}

	while (!g_gamma_regs_read_done && retry_count < AC247_GAMMA_COMPENSATION_READ_RETRY_MAX) {
		OPLUS_DSI_INFO("read gamma compensation regs, retry_count=%d\n", retry_count);
		memset(regs, 0, AC247_GAMMA_COMPENSATION_READ_LENGTH);
		memset(print_buf, 0, OPLUS_DSI_CMD_PRINT_BUF_SIZE);
		...
		oplus_ddic_dsi_read_cmd(0xD1, AC247_GAMMA_COMPENSATION_READ_LENGTH, regs);
		cnt = 0;
		memset(print_buf, 0, OPLUS_DSI_CMD_PRINT_BUF_SIZE);
		for (index = 0; index < AC247_GAMMA_COMPENSATION_READ_LENGTH; index++) {
			cnt += snprintf(print_buf + cnt, 4, "%02X ", regs[index]);
			OPLUS_DSI_INFO("gamma read=[%d]\n", regs[index]);
		}
		OPLUS_DSI_INFO("read buf=[%s]\n", print_buf);

		if (!strncmp(regs, regs_base, AC247_GAMMA_COMPENSATION_READ_LENGTH) || strncmp(regs, regs_last, AC247_GAMMA_COMPENSATION_READ_LENGTH)) {
			OPLUS_DSI_WARN("gamma compensation regs is invalid, retry\n");
			memcpy(regs_last, regs, AC247_GAMMA_COMPENSATION_READ_LENGTH);
			retry_count++;
			continue;
		}
		g_gamma_regs_read_done = true;
...
int oplus_panel_ext_init(struct drm_crtc *crtc)
{
	...
	if (!strcmp("panel_ac180_p_3_a0020_dsi_cmd", ctx->panel_info.panel_name)
		...
		rc |= oplus_panel_gamma_compensation(dsi);
				...
				||!strcmp("panel_ac266_p_7_a0014_dsi_cmd", ctx->panel_info.panel_name)
				||!strcmp("ac262_p_7_a0014_dsi_cmd_panel", ctx->panel_info.panel_name)
				||!strcmp("panel_aa599_p_7_a0014_dsi_cmd", ctx->panel_info.panel_name)
					&& !g_gamma_regs_read_done) {
		rc |= oplus_panel_ac178_gamma_compensation(dsi);
	} else if (!strcmp("panel_ac247_p_7_a0014_dsi_cmd", ctx->panel_info.panel_name)
			&& !g_gamma_regs_read_done) {
		rc |= oplus_panel_ac247_gamma_compensation(dsi);
	}
#endif /* OPLUS_FEATURE_DISPLAY_MAINLINE*/
