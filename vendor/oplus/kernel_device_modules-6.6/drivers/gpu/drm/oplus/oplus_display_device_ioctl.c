...
int oplus_display_get_panel_name(void *data) {
	struct drm_crtc *crtc;
	struct mtk_drm_crtc *mtk_crtc = NULL;
	struct mtk_ddp_comp *comp = NULL;
	struct drm_device *ddev = get_drm_device();
	struct panel_name *p_name = data;
	char *lcm_name  = NULL;

	crtc = list_first_entry(&(ddev)->mode_config.crtc_list,
			typeof(*crtc), head);
	if (!crtc) {
		OPLUS_DSI_ERR("find crtc fail, p_name=%p\n", p_name);
		return -EINVAL;
	}

	mtk_crtc = to_mtk_crtc(crtc);
	if (!mtk_crtc || !mtk_crtc->panel_ext || !mtk_crtc->panel_ext->params) {
		OPLUS_DSI_ERR("falied to get crtc info\n");
		return -EINVAL;
	}
	...
	mtk_ddp_comp_io_cmd(comp, NULL, GET_PANEL_NAME, &lcm_name);
	if(lcm_name) {
		memcpy(p_name->name, lcm_name,
				strlen(lcm_name) > sizeof(p_name->name) ?
				sizeof(p_name->name) : strlen(lcm_name));
	} else {
		return -EINVAL;
	}
	OPLUS_DSI_INFO("get panel name: %s\n", p_name->name);
