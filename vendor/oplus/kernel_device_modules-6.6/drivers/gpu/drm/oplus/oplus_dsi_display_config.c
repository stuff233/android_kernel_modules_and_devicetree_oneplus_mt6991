...
u8 oplus_dsi_panel_get_panel_flag(void *dsi)
{
...
u8 oplus_dsi_panel_get_panel_id1(void *dsi)
{
	struct mtk_dsi *mtk_dsi = dsi;
	struct dsi_panel_lcm *ctx = NULL;
...
u8 oplus_dsi_panel_get_panel_id2(void *dsi)
{
	struct mtk_dsi *mtk_dsi = dsi;
	struct dsi_panel_lcm *ctx = NULL;
...
u8 oplus_dsi_panel_get_panel_id3(void *dsi)
{
	struct mtk_dsi *mtk_dsi = dsi;
	struct dsi_panel_lcm *ctx = NULL;
...
u32 oplus_dsi_panel_get_panel_info(void *dsi)
{
	struct mtk_dsi *mtk_dsi = dsi;
...
int oplus_dsi_panel_parse_cmd_params(void *node_dev, void *lcm_ctx)
{
	...
	vdo_np = of_get_child_by_name(node, "oplus,vdo-aod-params");
	if(vdo_np) {
		OPLUS_DSI_INFO("timings_np is %s, node %s, vdo_np %s\n", timings_np->name, node->name, vdo_np->name);
		ret = oplus_dsi_panel_parse_vdo_param(vdo_np, ctx);
		if (ret < 0) {
			OPLUS_DSI_ERR("skip probe due to oplus prase vdo param error\n");
		}
	}
	of_node_put(timings_np);
	of_node_put(vdo_np);
...
{
	int rc = -EINVAL;
	int number = 0, len = 0, i, j, ret;
	char save[128] = { 0 };
	struct device_node *vdo_np, *child_np;
	struct dsi_panel_lcm *ctx = ctx_dev;
	struct vdo_aod_params *vdo_aod_config;
	u8 temp[sizeof(struct vdo_aod_cmds)] = {0};
	...
	vdo_aod_config = kzalloc(sizeof(struct vdo_aod_params) * (ctx->mode_num + 1), GFP_KERNEL);
	if(!vdo_aod_config) {
		OPLUS_DSI_ERR("vdo_aod_config kzalloc failed\n");
		rc = -EINVAL;
		return rc;
	}
	for_each_child_of_node(vdo_np, child_np) {
		OPLUS_DSI_INFO("node name %s\n", vdo_np->name);
		rc = of_property_read_u32(child_np, "porch_change_flag", &value);
		if (rc)
			OFP_INFO("failed to read porch_change_flag, rc=%d\n", rc);
		else
			vdo_aod_config[number].porch_change_flag = value;

		rc = of_property_read_u32(child_np, "dst-hfp", &value);
		if (rc)
			OFP_INFO("failed to read dst_hfp, rc=%d\n", rc);
		else
			vdo_aod_config[number].dst_hfp = value;

		rc = of_property_read_u32(child_np, "dst-vfp", &value);
		if (rc)
			OFP_INFO("failed to read dst_hfp, rc=%d\n", rc);
		else
			vdo_aod_config[number].dst_vfp = value;


		for (i = 0; i < MAX_DYN_CMD_NUM; i++) {
			ret = snprintf(save, sizeof(save),
				"oplus-vdo-aod-cmd-table%u",
				(unsigned int)i);
			if(ret < 0 || (size_t)ret >= sizeof(save))
				OPLUS_DSI_INFO("%s, %d, snprintf failed\n", __func__, __LINE__);

			len = of_property_read_variable_u8_array(child_np, save, &temp[0], 0,
						sizeof(struct vdo_aod_cmds));
			if(len < 0 || len > sizeof(struct vdo_aod_cmds)) {
				OPLUS_DSI_INFO("%s, %d: the %d vdo aod cmds of invalid cmd:%d\n",
					__func__, __LINE__, i, len);
				continue;
			} else if (len == 0) {
				continue;
			}
			vdo_aod_config[number].vdo_aod_cmd_table[i].cmd_num = temp[0];
			if(vdo_aod_config[number].vdo_aod_cmd_table[i].cmd_num  == 0)
				continue;
			for (j = 0; j < 64; j++) {
				vdo_aod_config[number].vdo_aod_cmd_table[i].para_list[j] =
					(unsigned char)temp[j + 1];
					OPLUS_DSI_INFO("list %d\n", vdo_aod_config[number].vdo_aod_cmd_table[i].para_list[j]);
			}
			OFP_INFO("cmd_num = %d\n", vdo_aod_config[number].vdo_aod_cmd_table[i].cmd_num);
		}
		OFP_INFO("flag %d porch %d,%d\n",
			vdo_aod_config[number].porch_change_flag,
			vdo_aod_config[number].dst_hfp,
			vdo_aod_config[number].dst_vfp);

		number++;
	}
	ctx->vdo_aod_config = vdo_aod_config;
	return rc;
}
...
bool oplus_dsi_display_video_panel(struct dsi_panel_lcm *ctx) {
	...
	if (!strcmp(panel_name, "panel_ab968_p_3_a0026_dsi_vdo") || !strcmp(panel_name, "panel_ae031_p_3_a0026_dsi_vdo")
		 || !strcmp(panel_name, "ac302_p_3_a0026_dsi_vdo_panel")) {
