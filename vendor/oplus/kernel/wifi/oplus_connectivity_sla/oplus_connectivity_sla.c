...
static int wlan_download_link = WLAN0_INDEX;
				...
				struct sk_buff *skb)
{
	...
	if (SLA_MODE_DUAL_WIFI == sla_work_mode &&
		...
		if (wlan_download_link == MAIN_WLAN) {
			wlan_download_link = SECOND_WLAN;
			choose_mark = MAIN_WLAN_MARK;
		} else if (wlan_download_link == SECOND_WLAN) {
			wlan_download_link = MAIN_WLAN;
			choose_mark = SECOND_WLAN_MARK;
		}
	}
