 ...
 *============================================================================
 ****************************************************************************/
#include "emiramainmipiraw_Sensor.h"
...
#define OTP_QCOM_PDAF_DATA_LENGTH 0x7F0

#define OTP_QCOM_PDAF_OFFSET_DATA_START_ADDR 0X0D30
#define OTP_QCOM_PDAF_OFFSET_DATA_LENGTH 0x336
...
static int emiramain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emiramain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emiramain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emiramain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emiramain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emiramain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emiramain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int emiramain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int emiramain_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static int emiramain_get_otp_qcom_pdaf_offset_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void calculate_prsh_length_lines(struct subdrv_ctx *ctx,
	struct mtk_hdr_ae *ae_ctrl,
	enum SENSOR_SCENARIO_ID_ENUM scenario_id);
/* STRUCT */

static struct eeprom_map_info emiramain_eeprom_info[] = {
	...
	{ EEPROM_META_AF_CODE, 0x0090, 0x0098, 0x0099, 6, true },
	{ EEPROM_META_AF_FLAG, 0x0096, 0x0010, 0x0099, 2, true },
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, emiramain_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, emiramain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, emiramain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, emiramain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, emiramain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, emiramain_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, emiramain_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, emiramain_set_awb_gain},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_DATA, emiramain_get_otp_qcom_pdaf_data},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_OFFSET_DATA, emiramain_get_otp_qcom_pdaf_offset_data},
};

static u32 emiramain_dcg_ratio_table_ratio4[] = {4000};
...
static struct subdrv_mode_struct mode_struct[] = {
	{/* B1-S1 4096x3072 @30fps QBIN(VBIN) with PDAF VB_max seamless A-1*/
		...
		.mode_setting_table = emiramain_preview_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = emiramain_seamless_preview,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_preview),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/* B1-S1 4096x3072 @30fps QBIN(VBIN) with PDAF VB_max seamless A-1*/
		...
		.mode_setting_table = emiramain_capture_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_capture_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = emiramain_seamless_capture,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_capture),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/*B2-S2 4096x2304_30FPS**/
		...
		.mode_setting_table = emiramain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_normal_video_setting),
	...
	{/* B3-1 4096x2304_60FPS */
		...
		.mode_setting_table = emiramain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{   /* G2 4096x2304 @60FPS QBIN DCG-HDR RAW12 w/ Partial-PD VB_max*/
		...
		.mode_setting_table = emiramain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		...
		.dcg_info = {
			...
			.dcg_gain_table = emiramain_dcg_ratio_table_ratio4,
			.dcg_gain_table_size = sizeof(emiramain_dcg_ratio_table_ratio4),
		},
		.dpc_enabled = true,
	...
	{/* V1  QBIN(VBIN)-V2H2 FHD 2048x1152_240FPS*/
		...
		.mode_setting_table = emiramain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* B6-S3 QBIN(VBIN)_4096x3072 @24FPS */
		...
		.mode_setting_table = emiramain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom2_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = emiramain_seamless_custom2,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_custom2),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/* F2-S1 8192x6144_30FPS remosaic*/
		...
		.mode_setting_table = emiramain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom3_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = emiramain_seamless_custom3,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_custom3),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/*L1-S1 QBIN(VBIN) 4096x3072_2-exp LBMF 30FPS with PDAF VB_max*/
		...
		.mode_setting_table = emiramain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom4_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = emiramain_seamless_custom4,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_custom4),
	...
	{/* F1-S1 QRMSC_4096x3072_30FPS for izoom--bayer */
		...
		.mode_setting_table = emiramain_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom5_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = emiramain_seamless_custom5,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_custom5),
		.hdr_mode = HDR_NONE,
		.pclk = 2196000000,
		.linelength = 11424,
		.framelength = 6400,
	...
	{/*V2 QBIN-V2H2 FHD 2048x1152_480FPS */
		...
		.mode_setting_table = emiramain_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom6_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*L2-S1 4096x3072 @30FPS Izoom 2exp-LBMF Qbayer w/ PDAF  VB_max seamless reg_A-1*/
		...
		.mode_setting_table = emiramain_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom7_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = emiramain_seamless_custom7,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_custom7),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 2196000000,
	...
	{/*B4 4096x3072 @60FPS QBIN(VBIN) with PDAF VB_max*/
		...
		.mode_setting_table = emiramain_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*G1-S2 4096x2304 30FPS DCG AP Merge*/
		...
		.mode_setting_table = emiramain_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
	...
	{/* G3 4096x2304 60FPS DCG AP Merge */
		...
		.mode_setting_table = emiramain_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom10_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_DCG_RAW,
	...
	{/* B8-S3 1856X1392,30.1fps,2X2Binning_Crop,w/All-PD(VBin) */
		...
		.mode_setting_table = emiramain_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom11_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* F2-S1 8192x6144_30FPS Qbayer*/
		...
		.mode_setting_table = emiramain_custom12_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom12_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = emiramain_seamless_custom12,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_custom12),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/*not use B6-S3 QBIN(VBIN)_4096x3072 @24FPS */
		...
		.mode_setting_table = emiramain_custom13_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom13_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* F1-S1 4096x3072 30FPS izoom - Qbayer*/
		...
		.mode_setting_table = emiramain_custom14_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom14_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*not use V1 QBIN(VBIN)-V2H2 FHD 2048x1152_240FPS*/
		...
		.mode_setting_table = emiramain_custom15_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom15_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* B5 4096x2304 @120FPS QBIN Partial-PD VB_max*/
		...
		.mode_setting_table = emiramain_custom16_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom16_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* G4 4096x3072 30FPS DCG AP Merge */
		...
		.mode_setting_table = emiramain_custom17_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom17_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_DCG_RAW,
	...
	{/*F4-S3 Izoom 2exp-LBMF 4096x3072 @24FPS w/ PDAF VB_max*/
		...
		.mode_setting_table = emiramain_custom18_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom18_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = emiramain_seamless_custom18,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_custom18),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 2196000000,
	...
	{/*L3-S3 4096X3072,24fps,2X2Binning_2exp_LB-MF,w/All-PD(VBin),For RST(9-16ms)*/
		...
		.mode_setting_table = emiramain_custom19_setting,
		.mode_setting_len = ARRAY_SIZE(emiramain_custom19_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = emiramain_seamless_custom19,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(emiramain_seamless_custom19),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 2196000000,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = emiramain_ana_gain_table,
	.ana_gain_table_size = sizeof(emiramain_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.init_setting_table = emiramain_init_setting,
	.init_setting_len = ARRAY_SIZE(emiramain_init_setting),
	.mode = mode_struct,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_AVDD1, {1800000, 1800000}, 3000},
...
const struct subdrv_entry emiramain_mipi_raw_entry = {
	.name = "emiramain_mipi_raw",
	.id = EMIRAMAIN_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_emiramain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != emiramain_eeprom_info[meta_id].meta)
		return -1;

	if (size != emiramain_eeprom_info[meta_id].size)
		return -1;

	addr = emiramain_eeprom_info[meta_id].start;
	readsize = emiramain_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, EMIRAMAIN_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
	...
	ret = adaptor_i2c_wr_p8(ctx->i2c_client, EMIRAMAIN_EEPROM_WRITE_ID >> 1,
			addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, EMIRAMAIN_EEPROM_WRITE_ID >> 1, reg, 0x0E);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, EMIRAMAIN_EEPROM_WRITE_ID >> 1, reg, 0x00);
	}
	...
	ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	UINT32 i = 0;
	kal_uint16 offset = 0;
	LOG_INF("write_Module_data start E");
	if(pStereodata != NULL) {
		...
		if (((pStereodata->uSensorId == EMIRAMAIN_SENSOR_ID) && ((data_length - 2) == CALI_DATA_MASTER_LENGTH))
				&& (data_base == EMIRAMAIN_STEREO_START_ADDR || data_base == EMIRAMAIN_STEREO_MT_START_ADDR
				|| data_base == EMIRAMAIN_STEREO_MT_105CM_START_ADDR)) {
		...
		} else if ((pStereodata->uSensorId == EMIRAMAIN_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
				&& (data_base == EMIRAMAIN_AESYNC_START_ADDR)) {
			...
			LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
				read_cmos_eeprom_8(ctx, EMIRAMAIN_AESYNC_START_ADDR),
				read_cmos_eeprom_8(ctx, EMIRAMAIN_AESYNC_START_ADDR+1),
				read_cmos_eeprom_8(ctx, EMIRAMAIN_AESYNC_START_ADDR+2),
				read_cmos_eeprom_8(ctx, EMIRAMAIN_AESYNC_START_ADDR+3),
				read_cmos_eeprom_8(ctx, EMIRAMAIN_AESYNC_START_ADDR+4),
				read_cmos_eeprom_8(ctx, EMIRAMAIN_AESYNC_START_ADDR+5),
				read_cmos_eeprom_8(ctx, EMIRAMAIN_AESYNC_START_ADDR+6),
				read_cmos_eeprom_8(ctx, EMIRAMAIN_AESYNC_START_ADDR+7));
		...
		LOG_INF("emiramain write_Module_data pStereodata is null\n");
		return -1;
	}
	return ret;
}
...
static int emiramain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case EEPROM_STEREODATA_MT_MAIN:
		read_emiramain_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MT_MAIN_105CM:
		read_emiramain_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA_105CM,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MW_MAIN:
	default:
		read_emiramain_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	}
	return 0;
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, EMIRAMAIN_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "emiramain read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "emiramain read_otp_info end\n");
}
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			printk("emiramain_mipi_raw i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",
...
static u16 emiramain_feedback_awbgain[] = {
	0x0B8E, 0x01,
	0x0B8F, 0x00,
	0x0B90, 0x02,
	0x0B91, 0x28,
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	emiramain_feedback_awbgain[5] = r_gain_int;
	emiramain_feedback_awbgain[7] = (r_gain - r_gain_int * 512) / 2;
	emiramain_feedback_awbgain[9] = b_gain_int;
	emiramain_feedback_awbgain[11] = (b_gain - b_gain_int * 512) / 2;
	subdrv_i2c_wr_regs_u8(ctx, emiramain_feedback_awbgain,
		ARRAY_SIZE(emiramain_feedback_awbgain));
}
...
void emiramain_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
		enum SENSOR_SCENARIO_ID_ENUM scenario_id,
...
int emiramain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	emiramain_get_min_shutter_by_scenario(ctx,
