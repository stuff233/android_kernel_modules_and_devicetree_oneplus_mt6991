...
static unsigned int do_single_lsc_targafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_targafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_targafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_targafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

//static unsigned int layout_check_targafront(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID, unsigned int *_cfg);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01c40055, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000530, 0x0000074C, do_single_lsc_targafront},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_targafront}, //Start address, block size is useless
		{0x00000001, 0x00000000, 0x00000000, do_pdaf_targafront},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00002000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_targafront}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT targafront_op_eeprom = {
	.name = "targafront_op_eeprom",
//	.check_layout_function = layout_check_targafront,
...
//static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &targafront_op_eeprom;

/*
unsigned int do_pdaf_targafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
...
#if 0
#define TARGAFRONT_HVBIN_PDAF_PROC1_SIZE  (0x1BA0-0x17D0) //976      0x3D0
#define TARGAFRONT_HVBIN_PDAF_PROC2_SIZE  (0x2208-0x1BB0) //1624     2600  0xA28
#define TARGAFRONT_HVBIN_PDAF_PROC1_ADDR  (0x17D0)
#define TARGAFRONT_HVBIN_PDAF_PROC2_ADDR  (0x1BB0)

//partial PD
#define TARGAFRONT_PARTIAL_PD_PROC1_SIZE  (0x3590-0x33A0) //496       0X1F0
#define TARGAFRONT_PARTIAL_PD_PROC2_SIZE  (0x398C-0x35A0) //1004      1500   0x5DC
#define TARGAFRONT_PARTIAL_PD_PROC1_ADDR  (0x33A0)
#define TARGAFRONT_PARTIAL_PD_PROC2_ADDR  (0x35A0)
unsigned int do_pdaf_targafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	return err;
	...
	debug_log("======================PDAF Data==================\n");
	debug_log("[%s] QPD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, TARGAFRONT_HVBIN_PDAF_PROC1_ADDR, TARGAFRONT_HVBIN_PDAF_PROC1_SIZE);
	debug_log("[%s] QPD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, TARGAFRONT_HVBIN_PDAF_PROC2_ADDR, TARGAFRONT_HVBIN_PDAF_PROC2_SIZE);
	debug_log("[%s] partial PD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, TARGAFRONT_PARTIAL_PD_PROC1_ADDR, TARGAFRONT_PARTIAL_PD_PROC1_SIZE);
	debug_log("[%s] partial PD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, TARGAFRONT_PARTIAL_PD_PROC2_ADDR, TARGAFRONT_PARTIAL_PD_PROC2_SIZE);
...
// partial PD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			TARGAFRONT_PARTIAL_PD_PROC1_ADDR + TARGAFRONT_PARTIAL_PD_PROC1_SIZE,
	...
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			...
			bios += TARGAFRONT_PARTIAL_PD_PROC1_SIZE;
	...
	// partial PD proc2
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				TARGAFRONT_PARTIAL_PD_PROC2_ADDR + TARGAFRONT_PARTIAL_PD_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
			...
			if (read_data_size > 0) {
				debug_log("[%s] partial PD proc2 First five %x, %x, %x, %x, %x\n",
					...
					pCamCalData->PDAF.Data[bios + 3],
					pCamCalData->PDAF.Data[bios + 4]);
				bios += TARGAFRONT_PARTIAL_PD_PROC2_SIZE;
...
// QPD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			TARGAFRONT_HVBIN_PDAF_PROC1_ADDR + TARGAFRONT_HVBIN_PDAF_PROC1_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc1 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += TARGAFRONT_HVBIN_PDAF_PROC1_SIZE;
			qpd_proc1_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
...
// QPD proc2
	if(qpd_proc1_flag) {
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				TARGAFRONT_HVBIN_PDAF_PROC2_ADDR + TARGAFRONT_HVBIN_PDAF_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc2 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += TARGAFRONT_HVBIN_PDAF_PROC2_SIZE;
			qpd_proc2_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		if (read_data_size > 0)	{
			...
			printk("targa5000k UnitR:%d, UnitG:%d, UnitB:%d, New Unit Max=%d",
					CalR, CalG, CalB, tempMax);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read CalGain Failed\n");
			printk("targa5000k fail UnitR:%d, UnitG:%d, UnitB:%d, New Unit Max=%d",
					CalR, CalG, CalB, tempMax);
			show_cmd_error_log(pCamCalData->Command);
		}
		...
		if (read_data_size > 0)	{
			...
			printk("targaGoldenR:%d, GoldenG:%d, GoldenB:%d, New Golden Max=%d",
					FacR, FacG, FacB, tempMax);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read FacGain Failed\n");
			printk("targafail GoldenR:%d, GoldenG:%d, GoldenB:%d, New Golden Max=%d",
					FacR, FacG, FacB, tempMax);
			show_cmd_error_log(pCamCalData->Command);
		}
...
static unsigned int layout_check_targafront(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID, unsigned int *_cfg)
{
	...
	if (check_id == 0x01c40055 || check_id == 0x01C3012B || check_id == 0x00510055) {	// hearder id on OTP guide
		...
		printk("targaheader_id matched 0x%08x\n", check_id);
		result = CAM_CAL_ERR_NO_ERR;
	} else{
		debug_log("header_id not matched 0x%08x\n", check_id);
		printk("targaheader_id not matched 0x%08x\n", check_id);
	}
	return result;
}*/
