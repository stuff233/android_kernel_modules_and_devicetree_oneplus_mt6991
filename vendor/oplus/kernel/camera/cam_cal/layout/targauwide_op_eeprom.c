...
static unsigned int do_single_lsc_targauwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_targauwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_targauwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

//static unsigned int layout_check_targauwide(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID, unsigned int *_cfg);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x0065009a, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000530, 0x0000074C, do_single_lsc_targauwide},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_targauwide},  //Start address, block size is useless
		...
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_targauwide}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT targauwide_op_eeprom = {
	.name = "targauwide_op_eeprom",
	//.check_layout_function = layout_check_targauwide,
...
//static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &targauwide_op_eeprom;
...
static unsigned int layout_check_targauwide(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID, unsigned int *_cfg)
{
	...
	if (check_id == 0x016B012B || check_id == 0x016d009a) {	// hearder id on OTP guide
		...
		printk("targaheader_id wide matched 0x%08x\n", check_id);
		result = CAM_CAL_ERR_NO_ERR;
	} else{
		debug_log("header_id not matched 0x%08x\n", check_id);
		printk("targaheader_id wide not matched 0x%08x\n", check_id);
	}
	return result;
}*/
