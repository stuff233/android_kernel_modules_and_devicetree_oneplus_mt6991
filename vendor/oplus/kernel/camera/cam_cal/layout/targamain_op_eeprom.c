...
static unsigned int do_single_lsc_targamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_targamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_targamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_targamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

//static unsigned int layout_check_targamain(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID, unsigned int *_cfg);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01480143, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00001140, 0x0000074C, do_single_lsc_targamain},
		{0x00000001, 0x00000020, 0x00000023, do_2a_gain_targamain}, //Start address, block size is useless
		{0x00000001, 0x00001890, 0x00000A28, do_pdaf_targamain},
		{0x00000000, 0x00000FAE, 0x00000A28, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_targamain}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT targamain_op_eeprom = {
	.name = "targamain_op_eeprom",
	//.check_layout_function = layout_check_targamain,
...
//static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &targamain_op_eeprom;

/*
unsigned int do_pdaf_targamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
...
#define TARGAMAIN_HVBIN_PDAF_PROC1_SIZE  (0x1C60-0x1890) //976      0x3D0
#define TARGAMAIN_HVBIN_PDAF_PROC2_SIZE  (0x22C8-0x1C70) //1624     2600  0xA28
#define TARGAMAIN_HVBIN_PDAF_PROC1_ADDR  (0x1890)
#define TARGAMAIN_HVBIN_PDAF_PROC2_ADDR  (0x1C70)

//partial PD
#define TARGAMAIN_PARTIAL_PD_PROC1_SIZE  (0x3720-0x3530) //496       0X1F0
#define TARGAMAIN_PARTIAL_PD_PROC2_SIZE  (0x3B1C-0x3730) //1004      1500   0x5DC
#define TARGAMAIN_PARTIAL_PD_PROC1_ADDR  (0x3530)
#define TARGAMAIN_PARTIAL_PD_PROC2_ADDR  (0x3730)
unsigned int do_pdaf_targamain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	struct STRUCT_CAM_CAL_DATA_STRUCT *pCamCalData =
	...
	debug_log("======================PDAF Data==================\n");
	debug_log("[%s] QPD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, TARGAMAIN_HVBIN_PDAF_PROC1_ADDR, TARGAMAIN_HVBIN_PDAF_PROC1_SIZE);
	debug_log("[%s] QPD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, TARGAMAIN_HVBIN_PDAF_PROC2_ADDR, TARGAMAIN_HVBIN_PDAF_PROC2_SIZE);
	debug_log("[%s] partial PD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, TARGAMAIN_PARTIAL_PD_PROC1_ADDR, TARGAMAIN_PARTIAL_PD_PROC1_SIZE);
...
// partial PD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			TARGAMAIN_PARTIAL_PD_PROC1_ADDR + TARGAMAIN_PARTIAL_PD_PROC1_SIZE,
	...
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			...
			bios += TARGAMAIN_PARTIAL_PD_PROC1_SIZE;
	...
	// partial PD proc2
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				TARGAMAIN_PARTIAL_PD_PROC2_ADDR + TARGAMAIN_PARTIAL_PD_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
			...
			if (read_data_size > 0) {
				debug_log("[%s] partial PD proc2 First five %x, %x, %x, %x, %x\n",
					...
					pCamCalData->PDAF.Data[bios + 3],
					pCamCalData->PDAF.Data[bios + 4]);
				bios += TARGAMAIN_PARTIAL_PD_PROC2_SIZE;
...
// QPD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			TARGAMAIN_HVBIN_PDAF_PROC1_ADDR + TARGAMAIN_HVBIN_PDAF_PROC1_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc1 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += TARGAMAIN_HVBIN_PDAF_PROC1_SIZE;
			qpd_proc1_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
...
// QPD proc2
	if(qpd_proc1_flag) {
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				TARGAMAIN_HVBIN_PDAF_PROC2_ADDR + TARGAMAIN_HVBIN_PDAF_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc2 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += TARGAMAIN_HVBIN_PDAF_PROC2_SIZE;
			qpd_proc2_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
