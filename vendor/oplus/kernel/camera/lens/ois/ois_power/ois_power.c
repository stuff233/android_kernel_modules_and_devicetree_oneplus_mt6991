...
static int ois2_power_enable(struct hf_device *hfdev, int sensor_type, int en)
{
	...
	if (en) {
		mutex_lock(&ois_power_dev->enable_mutex);
		ret = regulator_enable(ois_power_dev->vin);
		u32 min_vol = ois_power_dev->min_vol;
		u32 max_vol = ois_power_dev->max_vol;
		if ((min_vol != 0) && (max_vol != 0)) {
			LOG_INF("min_vol = %d,max_vol = %d", min_vol, max_vol);
			ret1 = regulator_set_voltage(ois_power_dev->vin, min_vol, max_vol);
			if (ret1) {
				LOG_INF("regulator_set_voltage(vin), min(%d - %d), ret(%d)(fail)\n",
					min_vol, max_vol, ret1);
			}
		} else {
...
static int parse_ois2_config(struct device *dev, struct ois_power_device *ois2_power_dev)
{
	int retval = 0;
	u32 value = 0;
	struct device_node *np = dev->of_node;
	LOG_INF("E");
	retval = of_property_read_u32(np, "ois2-min-vol", &value);
	if (retval < 0) {
		ois2_power_dev->min_vol = 0;
	} else {
		ois2_power_dev->min_vol = value;
	}

	retval = of_property_read_u32(np, "ois2-max-vol", &value);
	if (retval < 0) {
		ois2_power_dev->max_vol = 0;
	} else {
		ois2_power_dev->max_vol = value;
	}
	LOG_INF("min_vol = %d,max_vol = %d", ois2_power_dev->min_vol, ois2_power_dev->max_vol);
	LOG_INF("X");
	return retval;
}
static int ois_power_probe(struct i2c_client *client)
{
	...
	parse_ois2_config(dev, ois2_power_dev);
