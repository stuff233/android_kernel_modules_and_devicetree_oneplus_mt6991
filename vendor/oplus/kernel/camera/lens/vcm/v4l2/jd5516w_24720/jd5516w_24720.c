...
//  #include <soc/oplus/system/oplus_project.h>
 ...
 #define JD5516W_NAME				"jd5516w_24720"
 #define JD5516W_MAX_FOCUS_POS			1023
 #define JD5516W_ORIGIN_FOCUS_POS			0
 #define LOG_INF(format, args...)                                     \
        pr_info(JD5516W_NAME " [%s] " format, __func__, ##args)
 /*
 ...
 #define JD5516W_CONTROL_POWER_DOWN		1
 #define JD5516W_CONTROL_POWER_ON		0
 ...
 #define JD5516W_VIN_MIN             (2800000)
 #define JD5516W_VIN_MAX             (2800000)
  			      ...
  			      struct regval_list *vals, u32 len)
{
  	...
  	for (i = 0; i < len; i++) {
  		LOG_INF("Init write [%d, %d]", vals[i].reg_num, vals[i].value);
...
static int jd5516w_set_position(struct jd5516w_device *jd5516w, u16 val)
{
      ...
      if (loop_time >= 20) {
          LOG_INF("watting 0x05 flag timeout!");
      }

      res = (i2c_smbus_write_word_data(client, JD5516W_SET_POSITION_ADDR, swab16(val & 0x03FF)));
      if (!res) {
    	  jd5516w->focus->val = val;
      }
      return res;
}
...
static int jd5516w_release(struct jd5516w_device *jd5516w)
{
      ...
      if (-JD5516W_MOVE_STEPS < (val - JD5516W_MIDDLE_CODE) &&
        (val - JD5516W_MIDDLE_CODE) < JD5516W_MOVE_STEPS) {
        LOG_INF("jd5516w has been the pistion: %d", val);
      } else {
        ret = i2c_smbus_write_byte_data(client, JD5516W_CONTROL_REG,
                JD5516W_CONTROL_POWER_ON);
...
static int jd5516w_init(struct jd5516w_device *jd5516w)
{
      ...
      if (ret)
          return ret;
      jd5516w->active = true;
      LOG_INF("%s jd5516w driver init success.\n", __func__);
      return 0;
}
...
static int jd5516w_power_off(struct jd5516w_device *jd5516w)
{
  	...
  	if (ret)
  		LOG_INF("jd5516w release failed!\n");
...
static int jd5516w_power_on(struct jd5516w_device *jd5516w)
{
	...
	min = JD5516W_VIN_MIN;
	max = JD5516W_VIN_MAX;
	ret = regulator_set_voltage(jd5516w->vin, min, max);
	if (ret) {
...
static int jd5516w_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
  	...
  	LOG_INF("JD5516W driver IC version: 0x%x\n", ret);
      //get driver IC Info
  	ret = i2c_smbus_read_byte_data(client, 0x00);
  	LOG_INF("JD5516W driver IC Info: 0x%x\n", ret);
  	return 0;
}
...
static int jd5516w_vcm_suspend(struct jd5516w_device *jd5516w)
{
	int ret, val;
	unsigned long af_step = JD5516W_MOVE_STEPS;
	struct i2c_client *client = v4l2_get_subdevdata(&jd5516w->sd);
	...
	jd5516w_set_position(jd5516w, JD5516W_MIDDLE_CODE);
	ret = i2c_smbus_write_byte_data(client, JD5516W_CONTROL_REG, 0x01);
	if (ret) {
		LOG_INF("I2C failure!!!\n");
	} else {
		jd5516w->active = false;
...
static int jd5516w_vcm_resume(struct jd5516w_device *jd5516w)
{
	int ret = 0, val = JD5516W_MIDDLE_CODE, move_step = 0;
	struct i2c_client *client = v4l2_get_subdevdata(&jd5516w->sd);
	if (jd5516w->active)
		return 0;
	ret = i2c_smbus_write_byte_data(client, JD5516W_CONTROL_REG, 0x00);
	if (ret) {
		LOG_INF("I2C failure!!!\n");
		return ret;
	}

	LOG_INF("cur pos(%d), resume pos(%d)\n", val, resume_pos);
	move_step = resume_pos > val ? JD5516W_MOVE_STEPS : -JD5516W_MOVE_STEPS;

	while (resume_pos - val > JD5516W_MOVE_STEPS || val - resume_pos > JD5516W_MOVE_STEPS) {
		val += move_step;
		ret = jd5516w_set_position(jd5516w, val);
		if (ret)
			LOG_INF("I2C failure: %d", ret);
		usleep_range(JD5516W_MOVE_DELAY_US, JD5516W_MOVE_DELAY_US + 10);
	}
	ret = jd5516w_set_position(jd5516w, resume_pos);
...
static long jd5516w_ops_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	int ret = 0;
	struct jd5516w_device *jd5516w = sd_to_jd5516w_vcm(sd);
	struct i2c_client *client = v4l2_get_subdevdata(&jd5516w->sd);
	...
	case VCM_IOC_POWER_ON:
		ret = jd5516w_vcm_resume(jd5516w);
		LOG_INF("VCM_IOC_POWER_ON, cmd:%d\n", cmd);
		break;
	case VCM_IOC_POWER_OFF:
		ret = jd5516w_vcm_suspend(jd5516w);
...
static struct v4l2_subdev_core_ops jd5516w_ops_core = {
	.ioctl = jd5516w_ops_core_ioctl,
};

static const struct v4l2_subdev_ops jd5516w_ops = {
	.core = &jd5516w_ops_core,
};
...
static void jd5516w_remove(struct i2c_client *client)
{
...
static const struct of_device_id jd5516w_of_table[] = {
  	{ .compatible = "oplus,jd5516w_24720" },
  	{ },
};
...
MODULE_AUTHOR("JIAN LI <lijian@mediatek.com>");
