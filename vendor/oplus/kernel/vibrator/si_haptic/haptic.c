	...
	struct device_node *np)
{
	...
	sih_haptic->pinctrl = (struct pinctrl *)devm_pinctrl_get(dev);
	if(!IS_ERR_OR_NULL(sih_haptic->pinctrl)){
		sih_haptic->pinctrl_state = (struct pinctrl_state *)pinctrl_lookup_state(sih_haptic->pinctrl,
							"irq_active");
...
static int sih_f0_reg_and_value_config(sih_haptic_t *sih_haptic)
{
	...
	if (ret != 0) {
		hp_err("%s: f0_addr_regs acquire failed\n", __func__);
		return -EINVAL;
	}
	...
	if (ret != 0) {
		hp_err("%s: f0_addr_val acquire failed\n", __func__);
		return -EINVAL;
	}
	if (f0_reg_nums != f0_val_nums) {
		hp_err("%s: f0 addr nums acquire failed\n", __func__);
		return -EINVAL;
	}
