...
{
    if(0 != dump_kmsg(OPLUS_PARTITION_OPPORESERVE3_LINK, OPLUS_SHUTDOWN_KMSG_OFFSET, &shutdown_kmsg_dumper))
    {
        pr_err("dump kmsg to OPLUS_PARTITION_OPPORESERVE3_LINK failed\n");
...
static int shutdown_timeout_flag_write_now(void *args)
{
    ...
    opfile = filp_open(OPLUS_PARTITION_OPPORESERVE3_LINK, O_RDWR, 0600);

    if (IS_ERR(opfile)) {
        pr_err("open OPLUS_PARTITION_OPPORESERVE3_LINK error: %ld\n",PTR_ERR(opfile));
