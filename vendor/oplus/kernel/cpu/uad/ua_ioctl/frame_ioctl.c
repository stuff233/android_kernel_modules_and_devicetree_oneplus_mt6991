...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_GEAS)
void (*frame_update_geas_params)(struct hwmon_node_ext *src_node_ext) = NULL;
EXPORT_SYMBOL(frame_update_geas_params);

static long update_geas_node_ext_params(void __user *uarg)
{
	...
	struct hwmon_node_ext *node_ext = kzalloc(sizeof(struct hwmon_node_ext), GFP_KERNEL);
	if (node_ext != NULL) {
		node_ext->compute_record_count = info.compute_record_count;
		node_ext->frame_drive = info.frame_drive;
		node_ext->enable_irq = info.enable_irq;
		node_ext->active_ib_scale = info.active_ib_scale;
		node_ext->nactive_ib_scale = info.nactive_ib_scale;
		node_ext->active_ab_scale = info.active_ab_scale;
		node_ext->nactive_ab_scale = info.nactive_ab_scale;
		node_ext->active_sec_ib_scale = info.active_sec_ib_scale;
		node_ext->nactive_sec_ib_scale = info.nactive_sec_ib_scale;
		node_ext->active_sec_ab_scale = info.active_sec_ab_scale;
		node_ext->nactive_sec_ab_scale = info.nactive_sec_ab_scale;
		node_ext->active_voting_method = info.active_voting_method;
		node_ext->nactive_voting_method = info.nactive_voting_method;
		node_ext->hist_decay_rate = info.hist_decay_rate;
		node_ext->frame_debug_level = info.frame_debug_level;
		node_ext->sec_voting_enhanced = info.sec_voting_enhanced;
		node_ext->sec_io_pct_scale = info.sec_io_pct_scale;
		node_ext->slc_mpki_thres = info.slc_mpki_thres;
		memcpy(node_ext->sec_mbps_zones, info.sec_mbps_zones, sizeof(unsigned int) * DDR_OPP_CNT);

		if (frame_update_geas_params != NULL)
			frame_update_geas_params(node_ext);
...
ERROR_HANDLE:
	ofb_err("%s: kzalloc hwmon_node_ext fail, %d\n", __func__, ret);
	if (frame_update_geas_params != NULL)
		frame_update_geas_params(NULL);
...
static long handle_ofb_extra_cmd(unsigned int cmd, void __user *uarg)
{
	...
	case CMD_ID_UPDATE_GEAS_PARAMS:
		return update_geas_node_ext_params(uarg);
#endif
	default:
		return -ENOTTY;
	}
