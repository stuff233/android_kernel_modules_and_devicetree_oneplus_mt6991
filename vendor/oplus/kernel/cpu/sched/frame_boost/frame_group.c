...
unused_vutil:
	...
	if (unlikely(sysctl_frame_boost_debug & (DEBUG_SYSTRACE | DEBUG_SYSTRACE_KEY_UTIL)) && (grp != frame_boost_groups[SF_FRAME_GROUP_ID])) {
		if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
...
out:
	if (grp && unlikely(sysctl_frame_boost_debug & (DEBUG_SYSTRACE | DEBUG_SYSTRACE_KEY_UTIL))) {
		val_systrace_c(grp->id, ed_task_boost_type, "ed_task_type", ed_task_type);
	}
	return ret;
}
