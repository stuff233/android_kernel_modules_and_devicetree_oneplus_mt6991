...
struct kbase_context {
	...
	char fence_leader_comm[TASK_COMM_LEN];
...
#if IS_ENABLED(CONFIG_MALI_MTK_FW_ERR_RECOVERY_WA)
/**
 * If the event depends on FW, there is a chance to skip wait when FW is unresponsive.
 * In the case of conditon not met and FW unresponsive, treat it as timed out by returning 0;
 */
...
#if 1
/* for debug purpose, to find out scenarios timed out when FW is unresponsive*/
#undef wait_event_timeout
#undef wait_event_killable_timeout
...
({										\
	...
	if (!___wait_cond_timeout(condition))			\
		__ret = __wait_event_timeout(wq_head,				\
						condition,			\
					     timeout);				\
	if (__ret == 0 && kbdev->csf.firmware_unrecoverable)					\
	{									\
		dev_warn(kbdev->dev,"wait_event_timeout timed out");	\
	}									\
	__ret;									\
})
...
({										\
	...
	if (__ret == 0 && kbdev->csf.firmware_unrecoverable)						\
	{									\
		dev_warn(kbdev->dev,"wait_event_killable_timeout timed out");	\
	}									\
	__ret;									\
})
...
#endif
#endif
#endif /* CONFIG_MALI_MTK_FW_ERR_RECOVERY_WA */
