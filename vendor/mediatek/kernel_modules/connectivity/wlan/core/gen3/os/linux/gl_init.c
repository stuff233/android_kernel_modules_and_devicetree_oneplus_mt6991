...
#else
	.update_mgmt_frame_registrations = mtk_cfg80211_mgmt_frame_update,
#endif
...
static int wlanSetMacAddress(struct net_device *ndev, void *addr)
{
	...
	eth_hw_addr_set(prGlueInfo->prDevHandler, _addr);
#else
...
static struct wireless_dev *wlanNetCreate(PVOID pvData)
{
	...
	mtk_wcn_hif_sdio_get_dev(*((UINT32 *) pvData), &prDev);
#else
	prDev = pvData;
#endif
	if (!prDev)
...
void reset_p2p_mode(P_GLUE_INFO_T prGlueInfo)
{
	...
	rWlanStatus = kalIoctl(prGlueInfo, wlanoidSetP2pMode, (PVOID)&rSetP2P,
		sizeof(struct PARAM_CUSTOM_P2P_SET_STRUCT), FALSE, FALSE, TRUE, &u4BufLen);
...
INT_32 set_p2p_mode_handler(struct net_device *netdev,
	struct PARAM_CUSTOM_P2P_SET_STRUCT p2pmode)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			   wlanoidSetP2pMode,
			   &rSetP2P, sizeof(struct PARAM_CUSTOM_P2P_SET_STRUCT), FALSE, FALSE, TRUE, &u4BufLen);
...
#if KERNEL_VERSION(5, 9, 0) <= LINUX_VERSION_CODE
			if (prGlueInfo->prAdapter->rWifiVar.ucThreadScheduling == SCHED_NORMAL) {
				sched_set_normal(prGlueInfo->main_thread, prGlueInfo->prAdapter->rWifiVar.cThreadNice);
				sched_set_normal(prGlueInfo->hif_thread, prGlueInfo->prAdapter->rWifiVar.cThreadNice);
				sched_set_normal(prGlueInfo->rx_thread, prGlueInfo->prAdapter->rWifiVar.cThreadNice);
			} else if (prGlueInfo->prAdapter->rWifiVar.ucThreadScheduling == SCHED_FIFO) {
...
#if (KERNEL_VERSION(5, 16, 0) <= LINUX_VERSION_CODE)
				ether_addr_copy(_addr, MacAddr.sa_data);
				eth_hw_addr_set(prGlueInfo->prDevHandler, _addr);
#else
