...
#define TEMP_BUF_LEN 280
	...
	tval.tv_sec -= (time64_t)sys_tz.tz_minuteswest * 60;
	...
	i4TimeUsed = kalSnprintf(pucTemp, TEMP_BUF_LEN, "%04d-%02d-%02d %02d:%02d:%02d.%03d ",
				 ...
				 tm.tm_min, tm.tm_sec,
#if KERNEL_VERSION(5, 0, 0) <= LINUX_VERSION_CODE
				 (INT_32)(tval.tv_nsec / USEC_PER_MSEC / USEC_PER_MSEC)
#else
				 (INT_32)(tval.tv_usec / USEC_PER_MSEC)
#endif
				 );
	if (i4TimeUsed < 0) {
...
INT_32 procInitFs(VOID)
{
	...
	prEntry = proc_create(PROC_DBG_LEVEL_NAME, 0664, gprProcNetRoot, (const struct proc_ops *)&dbglevel_ops);
	if (!prEntry) {
	...
	prEntry = proc_create(PROC_AUTO_PERF_CFG, 0664, gprProcNetRoot, (const struct proc_ops *)&auto_perf_ops);
	if (!prEntry) {
	...
	fgDrvStatus = TRUE;
	prEntry = proc_create(PROC_DRV_STATUS, 0664, gprProcRoot, (const struct proc_ops *)&drv_status_ops);
	if (!prEntry) {
...
INT_32 procCreateFsEntry(P_GLUE_INFO_T prGlueInfo)
{
	...
	prEntry = proc_create(PROC_MCR_ACCESS, 0664, gprProcNetRoot, (const struct proc_ops *)&mcr_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR, "Unable to create /proc entry\n\r");
		return -1;
	}

	prEntry = proc_create(PROC_PKT_DELAY_DBG, 0664, gprProcNetRoot,
		(const struct proc_ops *)&proc_pkt_delay_dbg_ops);
	if (prEntry == NULL) {
	...
	prEntry = proc_create(PROC_SET_CAM, 0664, gprProcNetRoot, (const struct proc_ops *)&proc_set_cam_ops);
	if (prEntry == NULL) {
	...
	proc_set_user(prEntry, KUIDT_INIT(PROC_UID_SHELL), KGIDT_INIT(PROC_GID_WIFI));
#if CFG_SUPPORT_DEBUG_FS
	prEntry = proc_create(PROC_ROAM_PARAM, 0664, gprProcNetRoot, (const struct proc_ops *)&roam_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR, "Unable to create /proc entry\n\r");
		return -1;
	}
	prEntry = proc_create(PROC_COUNTRY, 0664, gprProcNetRoot, (const struct proc_ops *)&country_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR, "Unable to create /proc entry\n\r");
		return -1;
	}
...
#if CFG_SUPPORT_CFG_FILE
	prEntry = proc_create(PROC_SET_WIFI_CFG, 0664, gprProcNetRoot, (const struct proc_ops *)&wifi_cfg_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR, "Unable to create /proc entry\n\r");
		return -1;
	}
...
INT_32 cfgCreateProcEntry(P_GLUE_INFO_T prGlueInfo)
{
	...
	prEntry = proc_create(PROC_CFG_NAME, 0664, gprProcNetRoot, (const struct proc_ops *)&cfg_ops);
	if (prEntry == NULL) {
