...
{
	P_BSS_INFO_T prBssInfo;
	CMD_INDICATE_PM_BSS_CREATED rCmdIndicatePmBssCreated = {0};
...
{
	P_BSS_INFO_T prBssInfo;
	CMD_INDICATE_PM_BSS_CONNECTED rCmdIndicatePmBssConnected = {0};
...
{
	CMD_INDICATE_PM_BSS_ABORT rCmdIndicatePmBssAbort = {0};
...
{
	P_BSS_INFO_T prBssInfo;
	CMD_UPDATE_WMM_PARMS_T rCmdUpdateWmmParms = {0};
		 ...
		 IN PARAM_RATES_EX rSupportedRates, IN UINT_16 u2IELength, IN PUINT_8 pucIEBuf)
{
	...
	for (i = 0; i < prAdapter->rWlanInfo.u4ScanResultNum; i++) {
		...
		if (prAdapter->rWlanInfo.arScanResult[i].eOpMode == eOpMode &&
			...
			kalMemZero(&(prAdapter->rWlanInfo.arScanResult[i]), OFFSET_OF(PARAM_BSSID_EX_T, pucIE));

			/* then fill buffer */
			prAdapter->rWlanInfo.arScanResult[i].u4Length =
			    OFFSET_OF(PARAM_BSSID_EX_T, pucIE) + u2IELength;
