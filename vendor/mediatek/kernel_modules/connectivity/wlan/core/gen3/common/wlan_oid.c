		      ...
		      OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	if (prAdapter->rAcpiState == WLAN_ACPI_STATE_D3) {
		DBGLOG(OID, WARN,
	...
	if (prAdapter->fgIsRadioOff == FALSE && prAdapter->rWlanInfo.u4ScanResultNum > 0) {
		...
		for (i = 0; i < prAdapter->rWlanInfo.u4ScanResultNum; i++) {
			...
			kalMemCopy(prBssidEx,
				   &(prAdapter->rWlanInfo.arScanResult[i]), OFFSET_OF(PARAM_BSSID_EX_T, pucIE));
			   ...
			   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	PARAM_SSID_T rSsid[CFG_SCAN_SSID_MAX_NUM] = {0};
	PUINT_8 pucIe;
	UINT_8 ucSsidNum;
...
wlanoidSetBssid(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = MAC_ADDR_LEN;
	if (u4SetBufferLen != MAC_ADDR_LEN) {
		*pu4SetInfoLen = MAC_ADDR_LEN;
		return WLAN_STATUS_INVALID_LENGTH;
	} else if (prAdapter->rAcpiState == WLAN_ACPI_STATE_D3) {
		       ...
		       IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	CMD_SW_DBG_CTRL_T rCmdSwCtrl = {0};
			 ...
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	P_PARAM_NETWORK_ADDRESS_LIST prNetworkAddressList = pvSetBuffer;
	...
	*pu4SetInfoLen = 0;
	...
	prNetworkAddress = (P_PARAM_NETWORK_ADDRESS)(prNetworkAddressList + 1);
...
{
	WLAN_STATUS rStatus;
	CMD_TEST_CTRL_T rCmdTestCtrl = {0};
	...
	*pu4WritenLen = 0;
	...
	if (!kalStrnCmp(head, BATCHING_SET, kalStrLen(BATCHING_SET))) {
		...
		*pu4WritenLen = kalSnprintf(pvSetBuffer, 3, "%d", rCmdBatchReq.ucMScan);
		if (*pu4WritenLen >= 3) {
			DBGLOG(SCN, TRACE, "kalSnprintf failed\n");
			return -EINVAL;
		}
...
batchGetCmd(IN P_ADAPTER_T prAdapter,
	    OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	CMD_BATCH_REQ_T rCmdBatchReq = {0};
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
...
wlanoidSetMonitor(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	CMD_MONITOR_SET_INFO_T rCmdMonitorSetInfo = {0};
	WLAN_STATUS rWlanStatus = WLAN_STATUS_SUCCESS;
...
{
	struct CMD_LOWLATENCY_MODE_HEADER rLowLatencyModeHeader = {0};
...
#if (KERNEL_VERSION(5, 16, 0) <= LINUX_VERSION_CODE)
	ether_addr_copy(_addr, (PUINT_8)pvSetBuffer);
	eth_hw_addr_set(prGlueInfo->prP2PInfo->prDevHandler, _addr);
#else
...
uint32_t
wlanoidSetOffloadInfo(IN P_ADAPTER_T prAdapter,
			   IN void *pvSetBuffer, IN UINT_32 u4SetBufferLen,
			   OUT PUINT_32 pu4SetInfoLen)
{
	ASSERT(prAdapter);
	ASSERT(pu4SetInfoLen);
...
uint32_t
wlanoidQueryOffloadInfo(IN P_ADAPTER_T prAdapter,
			   IN void *pvSetBuffer, IN UINT_32 u4SetBufferLen,
			   OUT PUINT_32 pu4SetInfoLen)
{
	ASSERT(prAdapter);
