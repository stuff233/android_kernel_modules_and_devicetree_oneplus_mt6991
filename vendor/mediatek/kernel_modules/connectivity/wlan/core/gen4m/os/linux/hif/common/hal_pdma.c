				...
				uint32_t u4CurrTick, u_int8_t fgTimeout)
{
	...
	GLUE_DEC_REF_CNT(prAdapter->i4PwrCtrlBlockCnt);
		...
		enum ENUM_DRV_OWN_SRC eDrvOwnSrc)
{
	...
	GLUE_INC_REF_CNT(prAdapter->i4PwrCtrlBlockCnt);
...
{
	halSetFWOwnImpl(prAdapter, fgEnableGlobalInt, FALSE);
}

void halSetFWOwnImpl(struct ADAPTER *prAdapter,
		u_int8_t fgEnableGlobalInt,
		u_int8_t fgIsInSuspend)
{
	struct mt66xx_chip_info *prChipInfo;
	struct BUS_INFO *prBusInfo;
	struct GL_HIF_INFO *prHifInfo;
	...
	if (fgIsInSuspend) {
		if (!prAdapter->fgIsFwOwn)
			DBGLOG(HAL, TRACE,
				"Skip check if drv own before suspend\n");
		GLUE_INC_REF_CNT(prAdapter->i4PwrCtrlBlockCnt);
	} else if (p2pFuncNeedForceSleep(prAdapter)) {
	...
	if (GLUE_GET_REF_CNT(prAdapter->i4PwrCtrlBlockCnt) != 0) {
		DBGLOG(INIT, TRACE, "prAdapter->i4PwrCtrlBlockCnt = %d\n",
			prAdapter->i4PwrCtrlBlockCnt);
		goto unlock;
	}

	if (prAdapter->fgIsFwOwn == TRUE) {
		DBGLOG(INIT, LOUD, "Already FW own.\n");
		goto unlock;
	}
