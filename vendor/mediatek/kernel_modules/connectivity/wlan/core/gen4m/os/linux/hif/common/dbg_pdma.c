...
static u_int8_t halDumpHifDebugLog(struct ADAPTER *prAdapter)
{
	...
	if (readable == FALSE || dumpViaBt) {
		DBGLOG(HAL, ERROR, "PCIe not readable[%u][%u]",
		       readable, dumpViaBt);
		return FALSE;
	}
...
static u_int8_t halIsHistorySameToken(struct MSDU_TOKEN_HISTORY_INFO *prHistory)
{
...
{
	if (!prAdapter || !prHistory)
		return;

	if (!halIsHistorySameToken(prHistory))
		return;

	kalSendUevent(prAdapter, "abnormaltrx=DIR:TX,Event:Hang");
}

static void halNotifyTxHangDetailedEvent(struct ADAPTER *prAdapter,
				 struct MSDU_TOKEN_HISTORY_INFO *prHistory,
				 enum ENUM_OP_MODE eOPMode)
{
	uint8_t uevent[256];
	uint8_t *pos = uevent;
	uint8_t *end = uevent + sizeof(uevent);
	...
	pos += kalScnprintf(pos, end - pos,
		"abnormaltrx=DIR:TX,Event:Hang,OPMode:%u",
		eOPMode);
...
static bool halIsTxTimeout(struct ADAPTER *prAdapter, uint32_t *u4Token)
{
	...
	if (fgIsTimeout) {
		...
		if (prWifiVar->fgUseDetailedTxHangUEvent)
			halNotifyTxHangDetailedEvent(prAdapter, prHistory,
				 eOPMode);
		else
