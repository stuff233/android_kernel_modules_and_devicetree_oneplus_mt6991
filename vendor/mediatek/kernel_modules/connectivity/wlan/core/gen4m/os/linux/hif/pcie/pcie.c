...
static void mtk_pci_remove(struct pci_dev *pdev)
{
	...
	if (pdev == NULL) {
		DBGLOG(INIT, WARN, "pdev is NULL\n");
		return;
	}

	prDriverData = pci_get_drvdata(pdev);
	if (prDriverData == NULL) {
		DBGLOG(INIT, WARN, "prDriverData is NULL\n");
		return;
	}

	prChipInfo = prDriverData->chip_info;
	if (prChipInfo == NULL) {
		DBGLOG(INIT, WARN, "prChipInfo is NULL\n");
		return;
	}
...
static int mtk_pci_suspend(struct pci_dev *pdev, pm_message_t state)
{
	...
	*/
	while (wait < 500) {
		if ((prAdapter->i4PwrCtrlBlockCnt == 0) &&
