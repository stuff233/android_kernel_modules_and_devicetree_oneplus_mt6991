			 ...
			 struct cfg80211_wowlan *wow)
{
	...
	if (prGlueInfo && prGlueInfo->prAdapter) {
		...
		set_bit(SUSPEND_FLAG_CLEAR_WHEN_RESUME,
			&prAdapter->ulSuspendFlag);
		halSetSuspendFlagToFw(prAdapter, TRUE);
		/* Set FW own if host is going to suspend
		 * and the status is driver own.
		 */
		halSetFWOwnImpl(prAdapter, FALSE, TRUE);
	}
end:
	kalHaltUnlock();
