...
static u_int8_t isIfaceTypeSupportChannel(struct ADAPTER *prAdapter, uint32_t iface,
	uint32_t filter, enum ENUM_BAND eBand, uint8_t ucChannelNum,
	uint32_t u4channel_freq, uint8_t fgDFS, uint32_t *pau4SafeChnl)
{
	...
	if (iface & (IFACE_MODE_STA | IFACE_MODE_P2P_CLIENT | IFACE_MODE_TDLS |
		...
		if (eBand == BAND_5G) {
			if (rlmDomainIsIndoorChannel(
				...
				BAND_5G,
				ucChannelNum)) {
				DBGLOG(REQ, TRACE, "skip channels %d, indoor channel\n", u4channel_freq);
				result = FALSE;
			} else if (fgDFS) {
				DBGLOG(REQ, TRACE, "skip channels %d, dfs channel\n", u4channel_freq);
				result = FALSE;
			}
			if ((u2CountryCode == COUNTRY_CODE_TW) &&
				(u4channel_freq >= 5180 &&
				u4channel_freq <= 5260)) {
				DBGLOG(REQ, TRACE, "skip channels %d, country=%d\n", u4channel_freq, u2CountryCode);
				result = FALSE;
			}
		}
#if (CFG_SUPPORT_WIFI_6G == 1)
		if (eBand == BAND_6G) {
			...
			if (!IS_6G_PSC_CHANNEL(ucChannelNum)) {
				DBGLOG(REQ, TRACE, "skip channels %d, not psc channel\n", u4channel_freq);
				result = FALSE;
			}
					...
					u2CountryCode,
					PWR_MODE_6G_VLP,
					&vlp) != WLAN_STATUS_SUCCESS
					|| !vlp) {
				DBGLOG(REQ, TRACE, "skip channels %d, not vlp\n", u4channel_freq);
				result = FALSE;
			}
#else
			else if (u4channel_freq >= 6435) {
				DBGLOG(REQ, TRACE, "skip channels %d >= 6435\n", u4channel_freq);
				result = FALSE;
			}
		...
		if ((filter & CELLULAR_COEXISTENCE) && pau4SafeChnl) {
			if (!p2pFuncIsLteSafeChnl(
				...
				ucChannelNum,
				pau4SafeChnl)) {
				DBGLOG(REQ, TRACE, "skip channels %d, lte safe chnl\n", u4channel_freq);
				result = FALSE;
			}
		}
	}
...
#if (CFG_SUPPORT_REG_STAT_FROM_EMI == 1 && CFG_SUPPORT_STATS_ONE_CMD == 1)
void calAvgBitrate(
	struct ADAPTER *prAdapter,
...
{
	uint8_t i, ucBssIdx;
	struct EMI_BITRATE_INFO *prBitrateInfo;

	for (ucBssIdx = 0; ucBssIdx < MAX_BSSID_NUM &&
		ucBssIdx < ARRAY_SIZE(dest->au4CurrentBitrate); ucBssIdx++) {
		prBitrateInfo = &prAdapter->arBitrateInfo[ucBssIdx];
		if (!(IS_NET_ACTIVE(prAdapter, ucBssIdx) &&
			kalGetMediaStateIndicated(prAdapter->prGlueInfo,
				ucBssIdx) == MEDIA_STATE_CONNECTED &&
			prBitrateInfo->ucCnt != 0)) {
			continue;
		}

		for (i = 0; i < prBitrateInfo->ucCnt; i++) {
			DBGLOG(REQ, TRACE, "TPPD BSS(%u)[i][TP:PD]=[%u:%u]\n",
				ucBssIdx, i,
				prBitrateInfo->arBrHistory[i].u4CurrentBitrate,
				prBitrateInfo->arBrHistory[i].u4PredictBitrate);
			dest->au4CurrentBitrate[ucBssIdx] +=
				prBitrateInfo->arBrHistory[i].u4CurrentBitrate;
			dest->au4PredictBitrate[ucBssIdx] +=
				prBitrateInfo->arBrHistory[i].u4PredictBitrate;
		}

		dest->au4CurrentBitrate[ucBssIdx] /= prBitrateInfo->ucCnt;
		dest->au4PredictBitrate[ucBssIdx] /= prBitrateInfo->ucCnt;
	}
}
#endif
		...
		struct wireless_dev *wdev, const void *data, int data_len)
{
...
#if (CFG_SUPPORT_REG_STAT_FROM_EMI == 1 && CFG_SUPPORT_STATS_ONE_CMD == 1)
	struct PARAM_GET_STATS_ONE_CMD rParam;
	struct EVENT_STATS_LLS_TX_BIT_RATE rBrSrc;
#endif
...
#if (CFG_SUPPORT_REG_STAT_FROM_EMI == 1 && CFG_SUPPORT_STATS_ONE_CMD == 1)
	if (prAdapter->fgPredBrOffsetMapped) {
		rParam.u4Period = 1000;
		rStatus = kalIoctl(prGlueInfo,
			wlanoidQueryStatsOneCmd, &rParam,
			sizeof(rParam), &u4QueryInfoLen);
...
#else
	{
		...
		rStatus = kalIoctl(prGlueInfo,
				...
				u4QueryBufLen,
				&u4QueryInfoLen);
		DBGLOG(REQ, TRACE, "kalIoctl=%x, %u bytes", rStatus,
			u4QueryInfoLen);
	}
...
cal_avg_bitrate:
	if (prAdapter->fgPredBrOffsetMapped) {
		calAvgBitrate(prAdapter, &rBrSrc);
		forcePredBrIdx(prAdapter, &rBrSrc, &res);
	} else {
#else
	{
#endif
		...
		TAG_FOR_EACH(tag, u4QueryInfoLen, offset) {
			...
			case UNI_EVENT_STATISTICS_TAG_GET_BSS_PRED_TX_BITRATE: {
				...
				if (unlikely(ucBssIdx >=
					prAdapter->ucSwBssIdNum))
					break;

				forcePredBrIdxByBssEvt(prAdapter, tlv, &res,
					&ucAisIdx);
				break;
			}
			default:
				DBGLOG(REQ, WARN, "invalid tag:%u",
					TAG_ID(tag));
				break;
			}
		}
	...
	const void *data, int data_len)
{
	...
	DBGLOG(REQ, TRACE, "dbg: max_size = %d\n", max_size);
	...
	for (i = 0; i < ucNumOfChannel; i++) {
		...
		pr_channel_array->channel_array[i].channel_freq =
			u4channel_freq;
		DBGLOG(REQ, TRACE, "dbg: process channel: %d\n", u4channel_freq);
		// skip 2.4G if not included in band mask
		if (aucChannelList[i].eBand == BAND_2G4 &&
			!(band_mask & BAND_24GHZ)) {
			DBGLOG(REQ, TRACE, "skip 2G channel %d due to  band_mask=%d\n", u4channel_freq, band_mask);
			continue;
		}
		...
		if (aucChannelList[i].eBand == BAND_5G &&
			...
			!(band_mask & BAND_5GHZ)) {
			DBGLOG(REQ, TRACE, "skip 5G non DFS channel %d due to band_mask=%d\n", u4channel_freq, band_mask);
			continue;
		}

		// skip 5G DFS if not included in band mask
		if (aucChannelList[i].eBand == BAND_5G &&
			aucChannelList[i].fgDFS &&
			!(band_mask & BAND_5GHZ_DFS)) {
			DBGLOG(REQ, TRACE, "skip 5G DFS channel %d due to band_mask=%d\n", u4channel_freq, band_mask);
			continue;
		}
		...
		if (aucChannelList[i].eBand == BAND_6G &&
			!(band_mask & BAND_6GHZ)) {
			DBGLOG(REQ, TRACE, "skip 6G channel %d due to band_mask=%d\n", u4channel_freq, band_mask);
			continue;
		}
#endif /*CFG_SUPPORT_WIFI_6G*/
	...
	DBGLOG(REQ, TRACE, "sizeof(comb_matrix):%lu\n,",
		sizeof(struct ANDROID_USABLE_CHANNEL_UNIT)*
		usable_channel_nums + sizeof(u16));
