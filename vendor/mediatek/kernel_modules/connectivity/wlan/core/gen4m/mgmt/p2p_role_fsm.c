		...
		case P2P_ROLE_STATE_IDLE:
			...
			if (prP2pChnlReqInfo->fgIsChannelRequested) {
				...
				if (p2pFuncGetForceTrxConfig(prAdapter) ==
					...
					p2pFuncGetForceTrxConfig(prAdapter) ==
					P2P_FORCE_TRX_CONFIG_2T1R)
					p2pFuncSetApNss(prAdapter,
						prP2pRoleFsmInfo->ucBssIndex,
						2, 1);
#endif
...
//CONNECTIVITY.WIFI.HARDWARE.POWER.7757678, 2024/08/15, limitedly print debug info once in 10s .
...
//CONNECTIVITY.WIFI.HARDWARE.POWER.7757678, 2024/08/15, limitedly print debug info once in 10s .
		if (__ratelimit(&_rs)) {
			DBGLOG(P2P, INFO, "start timer %s", __func__);
		}
#endif
	}
#endif
