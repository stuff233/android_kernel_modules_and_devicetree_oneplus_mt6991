...
#if !CFG_SUPPORT_SKB_ALLOC_WORK
		/* SkbAllocWork call it later in wlanReturnPacket
		  * -DelaySetup
		  */
...
static void oplusWorkHandler(struct work_struct *work)
{
	struct GLUE_INFO *prGlueInfo;
	struct ADAPTER *prAdapter;
	if (mUevent[0] == '\0')
		return;
	prGlueInfo = CONTAINER_OF(work, struct GLUE_INFO, rOplusLpmEventWork);
	if (prGlueInfo == NULL) {
		return;
	}
	...
	kalSendUevent(prAdapter, mUevent);
}

int oplusLpmUeventInit(struct ADAPTER *prAdapter)
{
	struct GLUE_INFO *prGlueInfo;
	if (prAdapter == NULL) {
		return RET_ERR;
	...
	INIT_WORK(&prGlueInfo->rOplusLpmEventWork, oplusWorkHandler);
	return RET_OK;
}

void oplusConnUeventDeinit(struct ADAPTER *prAdapter)
{
	struct GLUE_INFO *prGlueInfo;
	if (prAdapter == NULL) {
		return;
	...
	cancel_work_sync(&prGlueInfo->rOplusLpmEventWork);
}

int oplusLpmSendUevent(struct ADAPTER *prAdapter, const char *src)
{
	struct GLUE_INFO *prGlueInfo;
	if (src == NULL || prAdapter == NULL) {
		return RET_ERR;
	}
	...
	schedule_work(&prGlueInfo->rOplusLpmEventWork);
	return RET_OK;
}
