...
#if CFG_SUPPORT_MBRAIN
uint32_t checkMbrOffset(struct ADAPTER *prAdapter,
	uint32_t num, struct MBRAIN_OFFSET_INFO *prOffsetInfo)
{
	uint32_t i;
	...
	for (i = 0; i < num; i++, prOffsetInfo++) {
		...
		if (prOffsetInfo->u4Tag == MBRAIN_EMI_OFFSET_PRED_BR)
			prAdapter->fgPredBrOffsetMapped = TRUE;
	}
					...
					uint8_t *pucEventBuf)
{
	...
	if (checkMbrOffset(prAdapter, num, prOffsetInfo) != WLAN_STATUS_SUCCESS)
		return WLAN_STATUS_FAILURE;
