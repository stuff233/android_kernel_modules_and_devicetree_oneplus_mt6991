	...
	uint8_t *buf, struct CMD_SET_BSS_INFO *cmd)
{
	...
	tag->u2Length = sizeof(*tag);
	tag->fgEnable =	!(cmd->ucBssColorInfo &
			  HE_OP_BSSCOLOR_BSS_COLOR_DISABLE);
	tag->ucBssColor = ((cmd->ucBssColorInfo & HE_OP_BSSCOLOR_BSS_COLOR_MASK)
...
#undef TEMP_LOG_TEMPLATE
	...
	for (i = 0; i < REG_STATS_STA_MAX_NUM; i++) {
		...
		prQueryStaStatistics->ucRespRcpi0 =
			prEmiStaStats->ucRespRcpi0;
		prQueryStaStatistics->ucRespRcpi1 =
			prEmiStaStats->ucRespRcpi1;
	...
	if (prAdapter->fgPredBrOffsetMapped)
		GET_MBR_EMI_FIELD(prAdapter, rStatus, arMbrBitrateInfo,
			prAdapter->arBitrateInfo);
#endif
