		...
		int delay, int retry, int pkt_type, bool flag)
{
	...
	if (event) {
		...
		if (pkt_type != BTMTK_TX_PKT_SEND_DIRECT_NO_ASSERT &&
			...
			BTMTK_WARN("%s: wait driver own retry, own_state[%d]", __func__, atomic_read(&cif_dev->own_state));
	...
	while (1) {
		...
		if (sent_len > 0) {
			...
			if (bdev && bdev->interface_state != BTMTK_STATE_DISCONNECT) {
				...
				if(ret<= 0){
					BTMTK_ERR("%s: tty write fail, ret = %d,zero_pkt_cnt= %d",__func__, ret, zero_pkt_cnt);
					usleep_range(1000, 1100);
				}
				log_time = jiffies_to_msecs(jiffies) - jiffies_to_msecs(log_time);
				if (log_time > TIME_DUMP_OF_FW_PKG_DL)
...
#endif
{
	...
	if (atomic_read(&cif_dev->own_state) == BTMTK_DRV_OWNING && count == 73 &&
		...
		BTMTK_INFO_RAW(data, count, "%s: recv 0x 0x 0x ...", __func__);
		data = data + 63;
		count = 10;
		BTMTK_INFO_RAW(data, count, "%s: data after trim 0x 0x 0x ... ", __func__);
	}
#endif
...
#endif
		__pm_relax(bt_trx_wakelock);
		BTMTK_INFO("%s: success (atomic), cmd[9] = 0x%02x", __func__, cmd[9]);
	}
unlock:
