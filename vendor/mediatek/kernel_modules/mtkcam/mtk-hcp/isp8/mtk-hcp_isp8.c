	...
	for (id = 0; id < block_num; id++) {
		if (str_mblock[id].is_dma_buf) {
			...
			case IMG_MEM_G_ID:
					...
					if (kref_read(&str_mblock[id].kref) > 0) {
						kref_get(&str_mblock[id].kref);
					} else {
	...
	for (id = 0; id < block_num; id++) {
		if (cap_mblock[id].is_dma_buf) {
			...
			case IMG_MEM_G_ID:
					...
					if (kref_read(&cap_mblock[id].kref) > 0) {
						kref_get(&cap_mblock[id].kref);
					} else {
	...
	for (id = 0; id < block_num; id++) {
		if (smvr_mblock[id].is_dma_buf) {
			...
			case IMG_MEM_G_ID:
					...
					if (kref_read(&smvr_mblock[id].kref) > 0) {
						kref_get(&smvr_mblock[id].kref);
					} else {
...
{
    struct kref *target_kref;
    void (*release_func)(struct kref *);
    const char *mode_str;

    if (mode == imgsys_streaming) {
        target_kref = &mb[IMG_MEM_G_ID].kref;
        release_func = gce_release_streaming;
        mode_str = "streaming";
    } else if (mode == imgsys_capture) {
        target_kref = &cmb[IMG_MEM_G_ID].kref;
        release_func = gce_release_capture;
        mode_str = "capture";
    } else {
        target_kref = &smb[IMG_MEM_G_ID].kref;
        release_func = gce_release_smvr;
        mode_str = "smvr";
    }
    // check the refcount is greater than 0
    if (kref_read(target_kref) > 0) {
        kref_put(target_kref, release_func);
		if (hcp_dbg_enable()) {
			pr_info("put-%s gce count(%d)\n", mode_str, kref_read(target_kref));
		}
    } else {
		if (hcp_dbg_enable()) {
        	pr_warn("Attempted to put GCE with zero refcount for %s mode\n", mode_str);
		}
    }
