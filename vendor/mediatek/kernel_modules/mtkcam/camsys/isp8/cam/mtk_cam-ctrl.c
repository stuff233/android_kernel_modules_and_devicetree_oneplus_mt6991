...
void mtk_cam_ctrl_stop(struct mtk_cam_ctrl *cam_ctrl)
{
	...
	if (ctx->is_pre_alloc_worker == true)
		kthread_flush_worker(ctx->flow_pre_worker);
	else
	...
	if (ctx->is_pre_alloc_worker == true)
	{
		kthread_flush_worker(ctx->done_pre_worker);
		kthread_flush_worker(ctx->sensor_pre_worker);
		kthread_flush_worker(ctx->tuning_pre_worker);
	}
	else
	{
