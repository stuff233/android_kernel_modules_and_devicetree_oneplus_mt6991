...
#define NUM_OF_KTHREAD_PRECREATE 3

struct kthread_worker g_sensor_worker[NUM_OF_KTHREAD_PRECREATE];
struct kthread_worker g_flow_worker[NUM_OF_KTHREAD_PRECREATE];
struct kthread_worker g_done_worker[NUM_OF_KTHREAD_PRECREATE];
struct kthread_worker g_tuning_worker[NUM_OF_KTHREAD_PRECREATE];
struct task_struct *g_sensor_worker_task[NUM_OF_KTHREAD_PRECREATE];
struct task_struct *g_flow_task[NUM_OF_KTHREAD_PRECREATE];
struct task_struct *g_done_task[NUM_OF_KTHREAD_PRECREATE];
struct task_struct *g_tuning_task[NUM_OF_KTHREAD_PRECREATE];
bool g_is_worker_alloc[NUM_OF_KTHREAD_PRECREATE] = {0};
#endif
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static int mtk_cam_ctx_fetch_daemon_workers(struct mtk_cam_ctx *ctx)
{
	if (ctx->stream_id >= NUM_OF_KTHREAD_PRECREATE)
	{
		dev_info(ctx->cam->dev, "ctx pre alloc workers stream_id %d out of range",  ctx->stream_id);
		ctx->is_pre_alloc_worker = false;
		return mtk_cam_ctx_alloc_workers(ctx);
	}
	if (g_is_worker_alloc[ctx->stream_id] == false)
	{
		dev_info(ctx->cam->dev, "ctx pre alloc workers err stream_id %d",  ctx->stream_id);
		ctx->is_pre_alloc_worker = false;
		return mtk_cam_ctx_alloc_workers(ctx);
	}
	ctx->is_pre_alloc_worker = true;
	ctx->sensor_worker_task = g_sensor_worker_task[ctx->stream_id];
	ctx->sensor_pre_worker = &g_sensor_worker[ctx->stream_id];
	ctx->flow_task = g_flow_task[ctx->stream_id];
	ctx->flow_pre_worker = &g_flow_worker[ctx->stream_id];
	ctx->done_task = g_done_task[ctx->stream_id];
	ctx->done_pre_worker = &g_done_worker[ctx->stream_id];
	ctx->tuning_task = g_tuning_task[ctx->stream_id];
	ctx->tuning_pre_worker = &g_tuning_worker[ctx->stream_id];

	return 0;
}

static int mtk_cam_ctx_alloc_daemons_workers(struct mtk_cam_ctx *ctx)
{
	int i = 0;
	unsigned int pre_stream_id = ctx->stream_id;

	for (i = 0; i < NUM_OF_KTHREAD_PRECREATE; i ++)
	{
		if (g_is_worker_alloc[i] == false)
		{
			ctx->stream_id = i;
			dev_info(ctx->cam->dev, "ctx alloc workers streamid %d",  ctx->stream_id);
			kthread_init_worker(&g_sensor_worker[i]);
			g_sensor_worker_task[i] =
				mtk_cam_ctx_create_task(ctx, "sensor_worker",
							&g_sensor_worker[i], true);
			if (!g_sensor_worker_task[i])
			{
				ctx->stream_id = pre_stream_id;
				return -1;
			}
			kthread_init_worker(&g_flow_worker[i]);
			g_flow_task[i] =
				mtk_cam_ctx_create_task(ctx, "camsys_worker",
							&g_flow_worker[i], true);
			if (!g_flow_task[i])
				goto fail_uninit_sensor_worker_task;

			kthread_init_worker(&g_done_worker[i]);
			g_done_task[i] =
				mtk_cam_ctx_create_task(ctx, "camsys_done",
								&g_done_worker[i], true);
			if (!g_done_task[i])
				goto fail_uninit_flow_worker_task;

			kthread_init_worker(&g_tuning_worker[i]);
			g_tuning_task[i] =
				mtk_cam_ctx_create_task(ctx, "camsys_tuning",
							    &g_tuning_worker[i], true);
			if (!g_tuning_task[i])
				goto fail_uninit_done_worker_task;

			g_is_worker_alloc[i] = true;
			ctx->stream_id = pre_stream_id;
			continue;

		fail_uninit_done_worker_task:
			kthread_stop(g_done_task[i]);
			g_done_task[i] = NULL;
		fail_uninit_flow_worker_task:
			kthread_stop(g_flow_task[i]);
			g_flow_task[i] = NULL;
		fail_uninit_sensor_worker_task:
			kthread_stop(g_sensor_worker_task[i]);
			g_sensor_worker_task[i] = NULL;
		ctx->stream_id = pre_stream_id;
		return -1;
		}
	}
...
{
	if (ctx->is_pre_alloc_worker == true)
	{
		ctx->sensor_worker_task = NULL;
		ctx->flow_task = NULL;
		ctx->done_task = NULL;
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
	if (mtk_cam_ctx_fetch_daemon_workers(ctx))
...
{
	if (ctx->is_pre_alloc_worker == true)
		return ctx_kthread_queue_work(ctx, ctx->sensor_pre_worker, work, __func__);
	else
...
{
	if (ctx->is_pre_alloc_worker == true)
		return ctx_kthread_queue_work(ctx, ctx->done_pre_worker, work, __func__);
	else
...
{
	if (ctx->is_pre_alloc_worker == true)
		return ctx_kthread_queue_work(ctx, ctx->flow_pre_worker, work, __func__);
	else
...
{
	if (ctx->is_pre_alloc_worker == true)
		return ctx_kthread_queue_work(ctx, ctx->tuning_pre_worker, work, __func__);
	else
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
	if (mtk_cam_ctx_alloc_daemons_workers(cam_dev->ctxs))
	{
		dev_err(dev, "%s: fail\n", __func__);
	}
#endif
