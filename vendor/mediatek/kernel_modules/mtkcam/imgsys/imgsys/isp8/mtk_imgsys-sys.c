...
static struct mtk_imgsys_dev *g_imgsys_dev = NULL;
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
	if (!IS_ERR_OR_NULL(s_daemon_info.daemon_task)) {
		power_task = s_daemon_info.daemon_task;
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
int imgsys_power_daemon_create(void)
{
	imgsys_daemon_create(&s_daemon_info, mtk_imgsys_worker_power_on, "imgsys_power_on_daemon", true);
	return 0;
}
#endif