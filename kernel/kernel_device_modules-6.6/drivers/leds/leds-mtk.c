...
bool oplus_project_silky_support(void) {
	return oplus_silky_support;
}
EXPORT_SYMBOL(oplus_project_silky_support);
...
#ifdef OPLUS_FEATURE_DISPLAY
	ret = fwnode_property_read_string(fwnode, "silky_brightness_support", &state);
	if (!ret) {
		if (!strncmp(state, "true", strlen("true")))
			oplus_silky_support = true;
		else
			oplus_silky_support = false;
	} else {
		oplus_silky_support = false;
	}
#endif /* OPLUS_FEATURE_DISPLAY */
