...
static int ISP_REGISTER_IRQ_USERKEY(char *userName)
{
	...
	if (FirstUnusedIrqUserKey == IRQ_USER_NUM_MAX) {
		...
		if (key < 0) {
			...
			strscpy_pad((void *)IrqUserKey_UserInfo[i].userName,
	...
	case ISP_CLEAR_IRQ: {
		if (copy_from_user(&ClearIrq, (void *)Param,
			...
			LOG_DBG(
			    ...
			    ClearIrq.EventInfo.St_type][ClearIrq.EventInfo.UserKey]);
			spin_lock_irqsave(
				...
				if (DebugFlag[0] == 1) {
					if (++bw_request[DebugFlag[1]] == 1) {
						mtk_pm_qos_add_request(
						    ...
						    MTK_PM_QOS_MM_MEMORY_BANDWIDTH_DEFAULT_VALUE);
					}
					Ret = ISP_SetPMQOS(
...
static inline void ISP_StopHW(int module)
{
	...
	switch (module) {
	case ISP_CAM_A_IDX:
	    strscpy_pad(moduleName, "CAMA", 5);
	    waitirq.Type = ISP_IRQ_TYPE_INT_CAM_A_ST;
		break;
	case ISP_CAM_B_IDX:
	    strscpy_pad(moduleName, "CAMB", 5);
	    waitirq.Type = ISP_IRQ_TYPE_INT_CAM_B_ST;
		break;
	default:
	    strscpy_pad(moduleName, "CAMC", 5);
		goto RESET;
	}
...
static void ISP_add_device_link(struct platform_device *pDev)
{
	...
	if (mtk_larb > mtk_larbs) {
		larb_num = mtk_larb;
		strscpy_pad(mtk_larb_str, "mediatek,larb", 14);
	} else {
		larb_num = mtk_larbs;
		strscpy_pad(mtk_larb_str, "mediatek,larbs", 15);
	}
...
static int ISP_probe(struct platform_device *pDev)
{
	...
	if ((strncmp(pDev->dev.of_node->name, "camsys1", strlen("camsys1")) == 0)
		&& of_property_read_bool(pDev->dev.of_node, "power-domains")) {
		isp_use_genpd = true;
...
)
{
	...
	ret = 0;
	module = -1;
	strscpy_pad(moduleName, pDev->dev.of_node->name, sizeof(moduleName)-1);
 ...
 *
 ******************************************************************************/
static int ISP_OpenToProc(struct inode *inode, struct file *file)
{
	LOG_NOTICE("Not implement");
	return 0;
}
 ...
 *
 ******************************************************************************/
static int CAMIO_OpenToProc(struct inode *inode, struct file *file)
{
	LOG_NOTICE("Not implement");
	return 0;
}
