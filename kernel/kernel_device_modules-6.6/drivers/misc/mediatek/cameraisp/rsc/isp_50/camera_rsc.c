...
signed int rsc_enque_cb(struct frame *frames, void *req)
{
	...
	struct RSC_Config __maybe_unused *pRscConfig;
...
signed int CmdqRSCHW(struct frame *frame)
{
	...
	cmdq_pkt_write(pkt, cmdq_base, RSC_RAND_PNLTY_GAIN_CTRL_1_HW,
			pRscConfig->RSC_RAND_PNLTY_GAIN_CTRL1, CMDQ_REG_MASK);
#endif
	cmdq_pkt_write(pkt, cmdq_base, RSC_DCM_CTL_HW, 0x1111, CMDQ_REG_MASK);

	cmdq_pkt_write(pkt, cmdq_base, RSC_START_HW, 0x1, CMDQ_REG_MASK);
	/* RSC Interrupt read-clear mode */

	cmdq_pkt_wfe(pkt, cmdq_event_id);
	cmdq_pkt_write(pkt, cmdq_base, RSC_START_HW, 0x0, CMDQ_REG_MASK);
	/* RSC Interrupt read-clear mode */
...
static long RSC_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	signed int __maybe_unused enqnum;
...
{
	struct RSC_USER_INFO_STRUCT __maybe_unused *pUserInfo;
	/*unsigned int Reg;*/
...
static const struct proc_ops rsc_dump_proc_fops = {
	.proc_open = proc_rsc_dump_open,
	.proc_read = seq_read,
};
...
static const struct proc_ops rsc_reg_proc_fops = {
	.proc_open = proc_rsc_reg_open,
	.proc_read = seq_read,
	.proc_write = rsc_reg_write,
};
#endif
