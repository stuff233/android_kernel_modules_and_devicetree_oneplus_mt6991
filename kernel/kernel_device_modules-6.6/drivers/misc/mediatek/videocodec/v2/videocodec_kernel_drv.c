...
static struct platform_device *Vcodec_plat_dev;
static struct device *Vcodec_plat_device;
...
struct mtk_vcodec_dev *video_dev;
...
void VdecDvfsAdjustment(void)
{
	...
	pr_debug("+%s,%d._diff:%d\n", __func__, __LINE__, _diff);
	gMMDFVFSMonitorCounts++;
}
...
void vdec_power_off(void)
{
	...
	pm = &video_dev->pm;
    if (pm->larbvdecs[0]) {
	ret = pm_runtime_put_sync(pm->larbvdecs[0]);
	if (ret)
...
void venc_power_on(void)
{
	...
	if (ret)
	    pr_info("%s,Failed to get venc larb", __func__);
    }
...
void venc_power_off(void)
{
    ...
    pm = &video_dev->pm;
    if (pm->larbvencs[0]) {
	ret = pm_runtime_put_sync(pm->larbvencs[0]);
	if (ret)
		pr_info("[error]%s,pm_runtime_put_sync ret:%d", __func__, ret);
    }
	mutex_unlock(&VencPWRLock);
...
static long vcodec_lockhw(unsigned long arg)
{
	...
	if (ret) {
		pr_debug("[ERROR] copy_from_user failed: %d\n", ret);
		return -EFAULT;
	}
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (eValRet == VAL_RESULT_INVALID_ISR) {
				ret = vcodec_lockhw_dec_fail(rHWLock,
					FirstUseDecHW);
				if (ret) {
					pr_debug("[ERROR] lockhw_dec_fail failed %d\n",
						ret);
					return -EFAULT;
				}
			...
			if (grVcodecDecHWLock.pvHandle == 0) {
				...
				pr_debug("+%s,%d,\n", __func__, __LINE__);
				ret = pm_runtime_get_sync(vcodec_device);
				if (ret)
				...
				if (rHWLock.bSecureInst == VAL_FALSE)
					video_dev->svp_mode = 0;
				else
					video_dev->svp_mode = 1;
				pr_debug("[lock]%s,svp_mode:%d,bSecureInst:%d", __func__,
					video_dev->svp_mode, rHWLock.bSecureInst);
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (eValRet == VAL_RESULT_INVALID_ISR) {
				ret = vcodec_lockhw_enc_fail(rHWLock,
					FirstUseEncHW);
				if (ret) {
					pr_debug("[ERROR] lockhw_enc_fail failed %d\n",
						ret);
					return -EFAULT;
				}
			} else if (eValRet == VAL_RESULT_RESTARTSYS) {
...
#endif
#if IS_ENABLED(CONFIG_PM)
						ret = pm_runtime_get_sync(vcodec_device2);
						if (ret)
							pr_info("[error]%s,pm_runtime_get_sync ret:%d", __func__, ret);
#else
#ifndef KS_POWER_WORKAROUND
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
...
#if IS_ENABLED(CONFIG_PM)
			ret = pm_runtime_put_sync(vcodec_device);
			if (ret)
				pr_info("[error]%s,pm_runtime_put_sync ret:%d", __func__, ret);
#else
#ifndef KS_POWER_WORKAROUND
		...
		if (grVcodecEncHWLock.pvHandle ==
			...
			if (rHWLock.eDriverType ==
				...
				/* turn venc power off */
#if IS_ENABLED(CONFIG_PM)
				ret = pm_runtime_put_sync(vcodec_device2);
				if (ret)
					pr_info("[error]%s,pm_runtime_put_sync ret:%d", __func__, ret);
#else
#ifndef KS_POWER_WORKAROUND
...
#if IS_ENABLED(CONFIG_MTK_PMQOS)
		mtk_pm_qos_update_request(&vcodec_qos_request2, (int)emi_bw);
#endif
		gVENCBWRequested = 1;
		}
		mutex_unlock(&EncPMQoSLock);
...
static long vcodec_get_secure_handle(struct device *dev, unsigned long arg)
{
	...
	pr_debug("%s, [b]rSecObj.shared_fd: %d\n", __func__, rSecObj.shared_fd);
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case VCODEC_MVA_ALLOCATION:
	{
		...
		ret =  vcodec_get_mva_allocation(Vcodec_plat_device, arg, vcodec_queue);
	...
	{
		mutex_lock(&vcodec_queue->dev_lock);
		ret = vcodec_get_mva_free(Vcodec_plat_device, arg, vcodec_queue);
		mutex_unlock(&vcodec_queue->dev_lock);
		if (ret) {
	...
	case VCODEC_CACHE_INVALIDATE_BUFF:
	{
		...
		ret = vcodec_cache_flush_buff(Vcodec_plat_device, arg, flush_dma_direction);
		if (ret) {
	...
	{
		ret = vcodec_get_secure_handle(Vcodec_plat_device, arg);
		pr_debug("[INFO] VCODEC_GET_SECURE_HANDLE");
		if (ret) {
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case VCODEC_FREE_NON_CACHE_BUFFER:
	{
		struct VAL_MEMORY_T mem_data;

		user_data_addr = (unsigned char *)arg;

		ret = (long)copy_from_user(&mem_data, user_data_addr,
	...
	{
		ret = vcodec_get_secure_handle(Vcodec_plat_device, arg);
		pr_debug("%s: %d [INFO] VCODEC_GET_SECURE_HANDLE", __func__, __LINE__);
		if (ret) {
...
static int vcodec_open(struct inode *inode, struct file *file)
{
	...
	Driver_Open_Count++;
	vcodec_queue = mtk_vcodec_mem_init(Vcodec_plat_device);
	...
	vcodec_queue->vcodec = Vcodec_plat_device;
...
static int vcodec_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    pr_debug("[VCODEC][] mmap region  Length(0x%lx), pfn(0x%lx)\n",
	...
	if (((length > VENC_REGION) || (pfn < VENC_BASE) ||
		...
		for (u4I = 0; u4I < VCODEC_INST_NUM_x_10; u4I++) {
			if ((ncache_mem_list[u4I].ulKVA != -1L) &&
				...
				ulSize =
				(ncache_mem_list[u4I].ulSize + 0x1000 - 1)
				& ~(0x1000 - 1);
...
static int vcodec_probe(struct platform_device *dev)
{
    ...
    Vcodec_plat_dev = dev;
    Vcodec_plat_device = &dev->dev;

	video_dev = devm_kzalloc(&dev->dev, sizeof(struct mtk_vcodec_dev), GFP_KERNEL);
	if (!video_dev)
		return -ENOMEM;
	video_dev->plat_dev = dev;
	...
	vcodec_class = class_create(VCODEC_DEVNAME);
	...
	vcodec_device = device_create(vcodec_class,
			NULL, vcodec_devno,
			NULL, VCODEC_DEVNAME);
	if (vcodec_device == NULL) {
		pr_debug("[VCODEC][ERROR] create vcodec_device fail");
		return ret;
	}
#if IS_ENABLED(CONFIG_PM)
	...
	vcodec_class2 = class_create(VCODEC_DEVNAME2);
	...
	if (vcodec_device2 == NULL) {
		pr_debug("[VCODEC][ERROR] create vcodec_device2 fail");
		return ret;
	}
	vcodec_device2->pm_domain = &mt_venc_pm_domain;
	ret = dma_set_mask_and_coherent(&dev->dev, DMA_BIT_MASK(34));
	dev_info(&dev->dev, "64-bit DMA enable\n");
	if (ret) {
		ret = dma_set_mask_and_coherent(&dev->dev, DMA_BIT_MASK(34));
		if (ret) {
	...
	if (!dev->dev.dma_parms) {
		dev->dev.dma_parms =
			devm_kzalloc(&dev->dev, sizeof(*dev->dev.dma_parms), GFP_KERNEL);
	}
	if (dev->dev.dma_parms) {
		ret = dma_set_max_seg_size(&dev->dev, (unsigned int)DMA_BIT_MASK(34));
		if (ret)
...
#endif
    pr_debug("vcodec_device:0x%p\n", vcodec_device);
    pr_debug("vcodec_device2:0x%p\n", vcodec_device2);
    pr_debug("Vcodec_plat_device:0x%p\n", Vcodec_plat_device);

	if (request_irq(VDEC_IRQ_ID,
		(irq_handler_t)video_intr_dlr,
		IRQF_NO_THREAD | IRQF_SHARED | IRQF_PROBE_SHARED, VCODEC_DEVNAME, video_dev) < 0)
	...
	if (request_irq(VENC_IRQ_ID,
		...
		IRQF_TRIGGER_HIGH, VCODEC_DEVNAME, video_dev) < 0)
	...
	ret = mtk_vcodec_init_pm(video_dev);
	if (ret) {
		pr_info("Failed to get mt vcodec clock source!");
		return ret;
	}
	...
	mtk_pm_qos_add_request(&vcodec_qos_request,
		MTK_PM_QOS_MM_MEMORY_BANDWIDTH, PM_QOS_DEFAULT_VALUE);
	mtk_pm_qos_add_request(&vcodec_qos_request2,
		MTK_PM_QOS_MM_MEMORY_BANDWIDTH, PM_QOS_DEFAULT_VALUE);
	mtk_pm_qos_add_request(&vcodec_qos_request_f,
		MTK_PM_QOS_VDEC_FREQ, PM_QOS_DEFAULT_VALUE);
	mtk_pm_qos_add_request(&vcodec_qos_request_f2,
		MTK_PM_QOS_VENC_FREQ, PM_QOS_DEFAULT_VALUE);
...
#if IS_ENABLED(CONFIG_MTK_PMQOS)
	ret = mmdvfs_qos_get_freq_steps(MTK_PM_QOS_VDEC_FREQ,
			&g_dec_freq_steps[0], &dec_step_size);
	if (ret < 0)
		pr_debug("Vdec get MMDVFS freq steps failed %d\n", ret);

	ret = mmdvfs_qos_get_freq_steps(MTK_PM_QOS_VENC_FREQ,
			&g_enc_freq_steps[0], &enc_step_size);
	if (ret < 0)
...
static int __init vcodec_driver_init(void)
{
	...
	}
	{
		...
		pr_debug("[VCODEC][DeviceTree] KVA_VENC_BASE(0x%p), KVA_VDEC_BASE(0x%p), KVA_VDEC_GCON_BASE(0x%p)",
