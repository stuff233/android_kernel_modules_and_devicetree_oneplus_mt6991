...
#define MAX_DEP_STR_LEN ((MAX_DEP_NUM + 1) * 7)
static void fbt_ux_set_cap(struct render_info *thr, int min_cap, int max_cap)
{
	int i;
	int local_dep_size = 0;
	char temp[7] = {0};
	char local_dep_str[MAX_DEP_STR_LEN] = {0};
	struct fpsgo_loading *local_dep_arr = NULL;
	int ret = 0;

	// Parameter validation
	if (!thr || min_cap < 0 || max_cap < min_cap) {
		pr_err("Invalid parameters in fbt_ux_set_cap\n");
		return;
	}

	local_dep_arr = kcalloc(MAX_DEP_NUM, sizeof(struct fpsgo_loading), GFP_KERNEL);
	if (!local_dep_arr) {
		pr_err("Failed to allocate memory for local_dep_arr\n");
		return;
	}

	local_dep_size = fbt_determine_final_dep_list(thr, local_dep_arr);
	if (local_dep_size > MAX_DEP_NUM) {
		pr_err("local_dep_size (%d) exceeds MAX_DEP_NUM (%d)\n", local_dep_size, MAX_DEP_NUM);
		goto out;
	}

	for (i = 0; i < local_dep_size; i++) {
		if (local_dep_arr[i].pid <= 0 || local_dep_arr[i].pid > 99999)
			continue;

		fbt_set_per_task_cap(local_dep_arr[i].pid, min_cap, max_cap, 1024);

		if (strlen(local_dep_str) == 0)
			ret = scnprintf(temp, sizeof(temp), "%d", local_dep_arr[i].pid);
		else
			ret = scnprintf(temp, sizeof(temp), ",%d", local_dep_arr[i].pid);

		if (ret > 0) {
			ssize_t remaining = MAX_DEP_STR_LEN - strlen(local_dep_str);
			if (remaining > 0)
				strscpy(local_dep_str + strlen(local_dep_str), temp, remaining);
			else
				pr_err("local_dep_str buffer full, truncating output\n");
		}
	}
