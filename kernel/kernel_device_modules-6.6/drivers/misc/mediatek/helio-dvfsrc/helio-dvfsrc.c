...
//#include "mtk_dvfsrc_reg_common.h"
#include "mtk_spm_vcore_dvfs_common.h"
#include "mtk_spm_vcore_dvfs_ipi_common.h"
#include "mtk_helio_dvfsrc_reg_mt6771.h"

#if IS_ENABLED(CONFIG_MTK_TINYSYS_SSPM_SUPPORT) && IS_ENABLED(CONFIG_MTK_TINYSYS_SSPM_V1)
#include "sspm_ipi.h"
...
static int commit_data(struct helio_dvfsrc *dvfsrc, int type, int data)
{
	...
	if (type == MTK_PM_QOS_EMI_OPP ||
		type == MTK_PM_QOS_VCORE_OPP ||
		type == MTK_PM_QOS_VCORE_DVFS_FIXED_OPP) {
	...
	switch (type) {
	case MTK_PM_QOS_MEMORY_BANDWIDTH:
		if (data == MTK_PM_QOS_MEMORY_BANDWIDTH_DEFAULT_VALUE)
			break;
...
#endif
		break;
	case MTK_PM_QOS_MD_PERI_MEMORY_BANDWIDTH:
		dvfsrc_write(dvfsrc, DVFSRC_SW_BW_4, data / 100);
		break;
	case MTK_PM_QOS_EMI_OPP:
		if (dvfsrc->log_mask & (0x1 << type))
			pr_info("[%s] class: %d, data: 0x%x\n",
				__func__, type, data);
	...
	case MTK_PM_QOS_VCORE_OPP:
		...
		break;
	case MTK_PM_QOS_VCORE_DVFS_FIXED_OPP:
		if (dvfsrc->log_mask & (0x1 << type))
			pr_info("[%s] class: %d, data: 0x%x\n",
					__func__, type, data);
		...
		if (data == VCORE_DVFS_OPP_NUM) { /* no fix opp*/
			...
			dvfsrc_write(dvfsrc, DVFSRC_BASIC_CONTROL,
				...
				| (1 << 15)));
#if IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT8788) && IS_ENABLED(CONFIG_MTK_SPM_V4)
			ret = wait_for_completion
		...
		unsigned long l, void *v)
{
	...
	commit_data(dvfsrc, MTK_PM_QOS_MEMORY_BANDWIDTH, l);
		...
		unsigned long l, void *v)
{
	...
	commit_data(dvfsrc, MTK_PM_QOS_CPU_MEMORY_BANDWIDTH, l);
		...
		unsigned long l, void *v)
{
	...
	commit_data(dvfsrc, MTK_PM_QOS_GPU_MEMORY_BANDWIDTH, l);
		...
		unsigned long l, void *v)
{
	...
	commit_data(dvfsrc, MTK_PM_QOS_MM_MEMORY_BANDWIDTH, l);
		...
		unsigned long l, void *v)
{
	...
	commit_data(dvfsrc, MTK_PM_QOS_MD_PERI_MEMORY_BANDWIDTH, l);
		...
		unsigned long l, void *v)
{
	...
	commit_data(dvfsrc, MTK_PM_QOS_EMI_OPP, l);
		...
		unsigned long l, void *v)
{
	...
	commit_data(dvfsrc, MTK_PM_QOS_VCORE_OPP, l);
		...
		unsigned long l, void *v)
{
	...
	commit_data(dvfsrc, MTK_PM_QOS_VCORE_DVFS_FIXED_OPP, l);
...
static void pm_qos_notifier_register(struct helio_dvfsrc *dvfsrc)
{
	...
	mtk_pm_qos_add_notifier(MTK_PM_QOS_MD_PERI_MEMORY_BANDWIDTH,
					&dvfsrc->pm_qos_md_peri_memory_bw_nb);
	mtk_pm_qos_add_notifier(MTK_PM_QOS_EMI_OPP, &dvfsrc->pm_qos_emi_opp_nb);
	mtk_pm_qos_add_notifier(MTK_PM_QOS_VCORE_OPP, &dvfsrc->pm_qos_vcore_opp_nb);
	mtk_pm_qos_add_notifier(MTK_PM_QOS_VCORE_DVFS_FIXED_OPP,
