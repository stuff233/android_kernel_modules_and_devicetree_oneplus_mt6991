...
static bool spm_sodi_is_not_gpt_event(
	struct wake_status *wakesta, long curr_time, long prev_time)
{
...
static inline bool spm_sodi_last_logout(long curr_time,
					long prev_time)
{
...
{
	static unsigned long sodi_logout_prev_time;
	...
	unsigned long sodi_logout_curr_time = 0;
	int need_log_out = 0;
...
unsigned int spm_go_to_sodi(u32 spm_flags, u32 spm_data, u32 sodi_flags)
{
	...
	//if (!(sodi_flags & SODI_FLAG_DUMP_LP_GS)) {
		...
		//}
	//}
#if !defined(SPM_K414_EARLY_PORTING) && IS_ENABLED(CONFIG_MTK_POWER_GS_LEGACY)
	if (sodi_flags & SODI_FLAG_DUMP_LP_GS)
