...
/* delete for Emira project low battery throttling function*/
...
/* start: add for Emira project low battery throttling function */
static unsigned int g_lbat_pmic_max_level = LOW_BATTERY_LEVEL_3; // default low_battery_level_3
static unsigned int g_lbat_pmic_level_num = (LOW_BATTERY_LEVEL_3 + 1); // default low_battery_level_num
static void set_low_battery_throttling_params(void)
{
	unsigned int project_id;

	project_id = get_project();
	pr_info("func:%s, project_id = %d\n", __func__, project_id);
	if (project_id == 24813) { // only target project used this params
		g_lbat_pmic_max_level = LOW_BATTERY_LEVEL_4;
		g_lbat_pmic_level_num = (LOW_BATTERY_LEVEL_4 + 1);
	}
}
/* end: add for Emira project low battery throttling function */
	...
	unsigned int input_lv)
{
	...
	if (intr_type == LBAT_INTR_1 || intr_type == LBAT_INTR_2) {
		if (input_lv < g_lbat_pmic_level_num  && temp_stage <= lbat_data->temp_max_stage)
			thl_lv = lbat_data->thl_lv[temp_stage * g_lbat_pmic_level_num + input_lv];
		else
		...
		if (input_lv && temp_stage <= lbat_data->temp_max_stage)
			thl_lv = lbat_data->thl_lv[temp_stage * g_lbat_pmic_level_num + g_lbat_pmic_max_level];
...
static int __used decide_and_throttle(enum LOW_BATTERY_USER_TAG user, unsigned int input, unsigned int thd_volt)
{
	...
	// unsigned int low_thd_volts[LBAT_PMIC_LEVEL_NUM] = {MIN_LBAT_VOLT+40, MIN_LBAT_VOLT+30, MIN_LBAT_VOLT+20, MIN_LBAT_VOLT+10};
	unsigned int low_thd_volts[LOW_BATTERY_LEVEL_4 + 1] = {MIN_LBAT_VOLT+40, MIN_LBAT_VOLT+30, MIN_LBAT_VOLT+20, MIN_LBAT_VOLT+10}; // default num set to be LOW_BATTERY_LEVEL_4 + 1
	...
	if (user == LBAT_INTR_1 || user == LBAT_INTR_2) {
		...
		if (lbat_data->lbat_thl_stop > 0 || lbat_data->ppb_mode == 1
			...
			lbat_data->l_pmic_lv = MAX(lbat_data->l_lbat_lv,
				lbat_data->lvsys_lv ? g_lbat_pmic_max_level : 0);
...
#endif
			dump_thd_volts_ext(&low_thd_volts[0], g_lbat_pmic_level_num);
		} else {
			lbat_thl_lv = convert_to_thl_lv(LBAT_INTR_1, lbat_data->temp_cur_stage, lbat_data->l_lbat_lv);
			lvsys_thl_lv = convert_to_thl_lv(LVSYS_INTR, lbat_data->temp_cur_stage, lbat_data->lvsys_lv);
			exec_throttle(MAX(lbat_thl_lv, lvsys_thl_lv), user, thd_volt, input);
		...
		const char *buf, size_t size)
{
	...
	unsigned int volt_l[LOW_BATTERY_LEVEL_4], volt_h[LOW_BATTERY_LEVEL_4]; // defualt set to be low_battery_level_4
	int intr_no;
	struct lbat_thd_tbl *thd_info;
	...
	volt_t_size = rearrange_volt(thd_info->lbat_intr_info, &volt_l[0], &volt_h[0], g_lbat_pmic_max_level);
...
static int check_duplicate(unsigned int *volt_thd)
{
	...
	for (i = 0; i < g_lbat_pmic_max_level; i++) {
		for (j = i + 1; j < g_lbat_pmic_max_level; j++) {
...
{
	int ret = 0, pt_power_off_lv = g_lbat_pmic_max_level;
...
static void psy_handler(struct work_struct *work)
{
	...
	if ((temp_stage <= lbat_data->temp_max_stage && temp_stage != lbat_data->temp_cur_stage) ||
		...
		if (lbat_data->ppb_mode != 1 && !lbat_data->lbat_thd_modify) {
			thl_lv_idx = lbat_data->temp_cur_stage * g_lbat_pmic_level_num + lbat_data->l_pmic_lv;
			pre_thl_lv = lbat_data->thl_lv[thl_lv_idx];
			thl_lv_idx = temp_stage * g_lbat_pmic_level_num + lbat_data->l_pmic_lv;
...
static int fill_thd_info(struct platform_device *pdev, struct lbat_thl_priv *priv, u32 *volt_thd, int intr_no)
{
	...
	max_thr_lv = g_lbat_pmic_max_level;
...
static int low_battery_thd_setting(struct platform_device *pdev, struct lbat_thl_priv *priv)
{
	...
	thl_level_size = g_lbat_pmic_level_num * (priv->temp_max_stage + 1);
	...
	volt_size = g_lbat_pmic_max_level * (priv->temp_max_stage + 1);
...
static int low_battery_throttling_probe(struct platform_device *pdev)
{
	...
	/* add for Emira project low battery throttling function */
	set_low_battery_throttling_params();
	...
	/* add for Emira project low battery throttling function */
	pr_info("project_id = %d, g_lbat_pmic_max_level = %d, g_lbat_pmic_level_num = %d\n", get_project(), g_lbat_pmic_max_level, g_lbat_pmic_level_num);
