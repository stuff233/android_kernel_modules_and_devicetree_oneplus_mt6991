...
static int oplus_select_ledmode(struct flashlight_dev *fdev, int ledmode)
{
	struct flashlight_dev_arg fl_dev_arg;
	if (!fdev || !fdev->ops) {
		pr_info("Failed with no flashlight ops\n");
	...
	fl_dev_arg.channel = fdev->dev_id.channel;
	fl_dev_arg.arg = ledmode;

	if (fdev->ops->flashlight_ioctl(OPLUS_FLASH_IOC_SELECT_LED_MODE,
				(unsigned long)&fl_dev_arg)) {
		pr_info("Failed to select led mode \n");
		return -EFAULT;
	}
		...
		struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case OPLUS_FLASH_IOC_SELECT_LED_MODE:
		pr_info("OPLUS_FLASH_IOC_SELECT_LED_MODE(%d,%d,%d): %d\n",
				type, ct, part, fl_arg.arg);
		mutex_lock(&fl_mutex);
		ret = oplus_select_ledmode(fdev, fl_arg.arg);
		mutex_unlock(&fl_mutex);
		break;
#endif /*OPLUS_FEATURE_CAMERA_COMMON*/
