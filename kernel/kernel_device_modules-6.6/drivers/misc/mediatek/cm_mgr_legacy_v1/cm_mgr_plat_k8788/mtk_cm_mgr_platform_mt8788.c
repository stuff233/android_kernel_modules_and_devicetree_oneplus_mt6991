...
#include "mtk_cm_mgr_mt8788.h"

#define CREATE_TRACE_POINTS
#include "mtk_cm_mgr_platform_events_mt8788.h"

#include <linux/fb.h>
#include <linux/notifier.h>
#include "mtk_disp_notify.h"

#if IS_ENABLED(CONFIG_MTK_DVFSRC_V1)
		...
		unsigned int cpu_id)
{
	...
	if (policy) {
		prev_freq_idx[cluster] = cpufreq_frequency_table_target(
			policy, frequency, CPUFREQ_RELATION_L);
		prev_freq[cluster] = frequency;
...
static void cm_mgr_ratio_timer_fn(struct timer_list *unused)
{
	...
	trace_CM_MGR__stall_ratio_0((unsigned int)cm_mgr_read(MP0_CPU_AVG_STALL_RATIO));
	trace_CM_MGR__stall_ratio_1((unsigned int)cm_mgr_read(CPU_AVG_STALL_RATIO));
...
void cm_mgr_perf_platform_set_force_status(int enable)
{
	if (enable) {
		...
		if (cm_mgr_perf_enable == 0)
			return;
#if IS_ENABLED(CONFIG_MTK_DVFSRC_V1)
		if ((cm_mgr_perf_force_enable == 0)||(pm_qos_update_request_status == 1))
			return;
		if (cm_mgr_idx == CM_MGR_LP4X_2CH_3600)
		...
		if ((cm_mgr_perf_force_enable == 0) ||
			...
			if (cm_mgr_idx == CM_MGR_LP4X_2CH_3600) {
				mtk_pm_qos_update_request(&ddr_opp_req,
						MTK_PM_QOS_EMI_OPP_DEFAULT_VALUE);
			} else if (cm_mgr_idx == CM_MGR_LP4X_2CH_3200) {
				mtk_pm_qos_update_request(&ddr_opp_req,
						MTK_PM_QOS_EMI_OPP_DEFAULT_VALUE);
			} else if (cm_mgr_idx == CM_MGR_LP3_1CH_1866) {
...
int cm_mgr_platform_init(void)
{
	...
	pr_info("[CM_MGR] register cpu tracepoint\n");
	ret = tracepoint_probe_register(cm_mgr_tracepoints[0].tp,
			cm_mgr_tracepoints[0].func,  NULL);
	if (ret)
		pr_info("[CM_MGR] cpu_frequency: Couldn't activate tracepoint\n");
	else
		cm_mgr_tracepoints[0].registered = true;
#endif /* CONFIG_MEDIATEK_CPU_DVFS */

#if IS_ENABLED(CONFIG_MTK_DVFSRC_V1)
	mtk_pm_qos_add_request(&ddr_opp_req, MTK_PM_QOS_EMI_OPP,
			MTK_PM_QOS_EMI_OPP_DEFAULT_VALUE);
#endif
...
void cm_mgr_set_dram_level(int level)
{
	...
	trace_CM_MGR__dvfsrc_set_power_model_ddr_request(dram_level);
#endif
}
