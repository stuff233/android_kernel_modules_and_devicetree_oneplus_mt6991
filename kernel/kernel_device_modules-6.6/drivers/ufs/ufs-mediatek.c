...
static void ufs_mtk_fix_refclk_26M(struct ufs_hba *hba);
...
static bool ufs_mtk_is_refclk_26M(struct ufs_hba *hba)
{
	struct ufs_mtk_host *host = ufshcd_get_variant(hba);

	return host->is_refclk_26M;
}

static void ufs_mtk_fix_refclk_26M(struct ufs_hba *hba)
{
	int err;
	u32 ref_clk = REF_CLK_FREQ_INVAL;
	struct ufs_mtk_host *host = ufshcd_get_variant(hba);

	err = ufshcd_query_attr_retry(hba, UPIU_QUERY_OPCODE_READ_ATTR,
			QUERY_ATTR_IDN_REF_CLK_FREQ, 0, 0, &ref_clk);

	dev_info(hba->dev, "query device ref clk = %d with err = %d\n", ref_clk, err);
	if (err || ref_clk != REF_CLK_FREQ_26_MHZ)
		return;

	host->is_refclk_26M = true;
}
...
static void ufs_mtk_trace_vh_compl_command_vend_ss(void *data, struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
...
static struct tracepoints_table interests[] = {
	...
	{
		.name = "android_vh_ufs_compl_command",
		.func = ufs_mtk_trace_vh_compl_command_vend_ss,
		.vend = UFS_VEND_SAMSUNG
	},
#endif
};
...
static void ufs_mtk_fix_clock_scaling(struct ufs_hba *hba)
{
	...
	if (hba->dev_info.wspecversion < 0x0400 || ufs_mtk_is_refclk_26M(hba)) {
		host->caps &= ~UFS_MTK_CAP_CLK_SCALE_ONLY_BY_USER;
...
static void ufs_mtk_fixup_dev_quirks(struct ufs_hba *hba)
{
	...
	if ((of_property_read_bool(np, "mediatek,ufs-vcc-always-on")) ||
	    (of_property_read_bool(np, "mediatek,ymtc-vcc-always-on") &&
	     (STR_PRFX_EQUAL("YMUSAB4TF3D1C1", dev_info->model) || STR_PRFX_EQUAL("YMUS9B4TF2D1C1", dev_info->model)))) {
...
#if defined(CONFIG_UFSFEATURE)
	if ((hba->dev_info.wmanufacturerid == UFS_VENDOR_SAMSUNG) && (evt == UFS_EVT_WL_SUSP_ERR) )
...
#if defined(CONFIG_UFSFEATURE)
static void ufs_mtk_exception_event_hook(struct platform_device *pdev)
{
...
skip_phy:
	...
	if (hba->dev_info.wmanufacturerid == UFS_VENDOR_SAMSUNG) {
		ufs_mtk_exception_event_hook(pdev);
	}
#endif
