...
#define OPLUS_CHRD_OV_THD 14500
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static int mt6375_charger_get_online(struct mt6375_chg_data *ddata, int *online_status)
{
	int ret;
	int attach;
	u32 val;

	ret = mt6375_chg_field_get(ddata, F_CHRDET_EXT, &val);
	attach = atomic_read(&ddata->attach[ddata->active_idx]);
	if (!ret && !val && attach) {
		*online_status = val;
		dev_info(ddata->dev,"%s, chrdet:%d attach:%d. \n", __func__, val, attach);
	} else {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	mt6375_charger_get_online(ddata, &attach);
#else
	attach = atomic_read(&ddata->attach[ddata->active_idx]);
#endif
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		mutex_lock(&ddata->attach_lock);
#ifdef OPLUS_FEATURE_CHG_BASIC
		mt6375_charger_get_online(ddata, &val->intval);
#else
...
int mt6375_reset_hvdcp_reg(struct mt6375_chg_data *ddata, bool en)
{
	if (!ddata)
		return -EINVAL;
	/* when unattach that en is false, need to pull low on dp/dm */
	if (en) {
...
static irqreturn_t mt6375_int_chrdet_ext_handler(int irq, void *data)
{
	...
	mt_dbg(ddata->dev, "++\n");
	ret = iio_read_channel_processed(&ddata->iio_adcs[ADC_CHAN_PDVBUS], &pdvbus);
	if (ret)
		dev_err(ddata->dev, "[%s] Failed to read pdvbus voltage\n", __func__);
	pdvbus = U_TO_M(pdvbus);
	dev_info(ddata->dev, "[%s] pd_vbus = %d", __func__, pdvbus);
	...
	if (!val) {
		...
		if (pdvbus > OPLUS_CHRD_UV_THD) {
			dev_info(ddata->dev, "[%s] hardware ovp\n", __func__);
			ddata->pdvbus_ovp_status = true;
		} else {
			ddata->pdvbus_ovp_status = false;
		}
		if (ddata->hvdcp_cfg_9v_done == true) {
			ddata->hvdcp_detach_time = local_clock() / 1000000;
		}
		...
		if (ddata->pdvbus_ovp_status) {
			dev_info(ddata->dev, "[%s] hardware ovp before, set online\n", __func__);
			mt6375_chg_attach_pre_process(ddata, ATTACH_TRIG_TYPEC, val);
			ddata->pdvbus_ovp_status = false;
		}
	}
...
/* oplus add for uvlo & ov */
	ret = mt6375_chg_field_set(ddata, F_CHRD_UV_VREF, OPLUS_CHRD_UV_THD);
	if (ret < 0) {
		dev_err(ddata->dev, "failed to set chrdet uv level\n");
		return ret;
	}
	ret = mt6375_chg_field_set(ddata, F_CHRD_OV_VREF, OPLUS_CHRD_OV_THD);
	if (ret < 0) {
