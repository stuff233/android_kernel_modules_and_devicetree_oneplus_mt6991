...
/* check board info */
static int check_board_info(void)
{
	int board_result = 0; // default is not wanted board id
	int board_id;

	board_id = get_PCB_Version();
	printk("%s: board id = %d\n", __func__, board_id);
	switch (board_id) {
	case EVT2: // EVT2
	    board_result = 1;
...
static int mt6379_charger_init_setting(struct mt6379_charger_data *cdata)
{
	...
	/* add for Emira VS1 BYP solution */
	project_id = get_project();
	board_result = check_board_info();
	dev_err(cdata->dev, "project = %d\n", project_id);
	if ((project_id == 24813) && (board_result == 1)) {
		ret = regmap_update_bits(cdata->rmap, 0x210, 0x07, 0);// MT6379 Pre-UV To 2.4v
		if (ret) {
			dev_info(dev, "%s, Failed to set pre-UV 2.4V\n\n", __func__);
		}
	}
