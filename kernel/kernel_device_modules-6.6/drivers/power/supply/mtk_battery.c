...
#ifndef CONFIG_OPLUS_MTK_BATTERY_V2
...
#if IS_ENABLED(CONFIG_OPLUS_CHG_V2) && !IS_ENABLED(CONFIG_OPLUS_CHARGER_OPTIGA)
...
#endif /* CONFIG_OPLUS_MTK_BATTERY_V2 */
#endif /* OPLUS_FEATURE_CHG_BASIC */

#ifdef OPLUS_FEATURE_CHG_BASIC
#ifdef CONFIG_OPLUS_MTK_BATTERY_V2
#include <linux/gpio.h>
...
int battery_type_check(int *battery_type)
{
	...
	if (batt_id == NULL) {
		bm_debug(gmb, "[battery_type_check]: batt_id iio channel is null []\n");
...
#else
static int oplus_batid_get_cmdline(void)
{
	...
	if (np) {
		...
		if (!bootparams || ret < 0) {
			bm_err(gmb, "failed to get bootargs property");
			return -1;
		}
		str = strstr(bootparams, "battery_res=");
		if (str) {
			str += strlen("battery_res=");
			ret = get_option(&str, &batid);
			if (ret == 1) {
				bm_err(gmb, "%s: batid: %u\n", __func__, batid);
				return batid;
			}
		}
	}
	bm_err(gmb, "%s: failed to get batid \n", __func__);
	return -1;
}
...
int fgauge_get_profile_id(struct mtk_battery *gm)
{
	...
	if (external_authenticate_support) {
		...
		if (battery_id >= 0) {
			bm_err(gmb, "%s: get battid suc from comline, battid %d\n", __func__, battery_id);
			return battery_id;
		}
		bm_err(gmb, "%s, external_authenticate_support,return battery_id 0\n", __func__);
		battery_id = 0;
		return battery_id;
	}
...
#if defined(OPLUS_FEATURE_CHG_BASIC) && !defined(CONFIG_OPLUS_MTK_BATTERY_V2)
/* only add for 0.1 precision battery temp*/
...
#ifdef CONFIG_OPLUS_MTK_BATTERY_V2
	fg_read_dts_val(gm, np, "RBAT_PULL_DOWN_R", &(gm->rbat.rbat_pull_down_r), 1);
	fg_read_dts_val(gm, np, "NTC_SERIES_R", &(gm->rbat.ntc_series_r), 1);
#endif
...
{
	int bat_volt = 0;
	bat_volt = gauge_get_int_property(oplus_gm, GAUGE_PROP_BATTERY_VOLTAGE);
	bat_volt -= 10;
	return bat_volt;
}
...
{
	int bat_current = 0;
	bat_current = gauge_get_int_property(oplus_gm, GAUGE_PROP_BATTERY_CURRENT);
	bat_current = 0 - bat_current / 10;
...
bool meter_fg_30_get_battery_authenticate(void)
{
	...
	if (external_authenticate_support) {
		bm_err(oplus_gm, "%s, external_authenticate_support,return hmac value\n", __func__);
		return oplus_get_hmac();
	}

	battery_id = fgauge_get_profile_id(oplus_gm);
...
static bool meter_set_gauge_power_sel(int sel)
{
	...
	return set_charge_power_sel(oplus_gm, chgsel);
}
...
static bool meter_set_gauge_aging(int sel)
{
	if (sel >= AGING_LOW_LIMIT && sel <= AGING_HIGH_LIMIT) {
		bm_err(oplus_gm,
			"exe_BAT_EC cmd,change aging to=%d\n", sel);
		wakeup_fg_algo_cmd(oplus_gm,
			FG_INTR_KERNEL_CMD,
...
#define CYCLE_HIGH_LIMIT 5000
static bool meter_set_gauge_cycles(int sel)
{
	if (sel <= CYCLE_HIGH_LIMIT) {
		bm_err(oplus_gm,
			"exe_BAT_EC cmd,change cycles to=%d\n", sel);
		wakeup_fg_algo_cmd(oplus_gm,
			FG_INTR_KERNEL_CMD,
...
static int meter_fg_30_get_batt_qmax(int *qmax1, int *qmax2)
{
	if (qmax1 == NULL || qmax2 == NULL) {
		bm_err(oplus_gm, "%s qmax1 or qmax2 is NULL\n", __func__);
		return -EINVAL;
	}
	if (oplus_gm == NULL) {
		bm_err(oplus_gm, "%s oplus_gm is NULL\n", __func__);
		return -EINVAL;
	}
...
{
	if (car_c == NULL) {
		bm_err(oplus_gm, "%s car_c is NULL\n", __func__);
		return -EINVAL;
	}
	if (oplus_gm == NULL) {
		bm_err(oplus_gm, "%s oplus_gm is NULL\n", __func__);
	...
	struct gauge_cali_track_state *state, int track_reason)
{
	...
	if (!state->pre_info) {
		bm_err(oplus_gm, "[%s]: gauge_cali_track alloc error.\n", __func__);
...
bool gauge_cali_track_check_state(struct gauge_cali_track_state *state, int offset)
{
	...
	if (!(state->end_flag ^ GAUGE_TRACK_STATE_END_MASK)) {
		bm_info(oplus_gm, "[%s]: gauge_cali_track_state end_flag complete.\n", __func__);
		return true;
	} else if (ktime_ms_delta(now, state->begin_time) > GAUGE_TRACK_STATE_UPDATE_TIMEOUT_MS) {
		bm_err(oplus_gm, "[%s]: gauge_cali_track_state update timeout.\n", __func__);
		return true;
	}
	return false;
}
...
/*add for distinguish fuelgague and outlay-gague*/
	fg_read_dts_val(gm, pdev->dev.of_node, "FUELGAGUE_APPLY", &(fuelgauge_apply), 1);
	bm_err(gm, "%s, fuelgauge_apply:%d\n", __func__, fuelgauge_apply);

	fg_read_dts_val(gm, pdev->dev.of_node, "IS_SUBBOARD_TEMP_SUPPORT", &(is_subboard_temp_support), 1);
	bm_err(gm, "%s, is_subboard_temp_support:%d\n", __func__, is_subboard_temp_support);

	fg_read_dts_val(gm, pdev->dev.of_node, "Enable_Is_Force_Full", &(enable_is_force_full), 1);
	bm_err(gm, "%s, enable_is_force_full:%d\n", __func__, enable_is_force_full);

	fg_read_dts_val(gm, pdev->dev.of_node, "USE_MORE_PRECISE_NTC_TABLE", &(use_more_precise_NTC_table), 1);
	bm_err(gm, "%s, use_more_precise_NTC_table:%d\n", __func__, use_more_precise_NTC_table);

	fg_read_dts_val(gm, pdev->dev.of_node, "EXTERNAL_AUTHENTICATE", &(external_authenticate_support), 1);
	bm_err(gm, "%s, external_authenticate_support:%d\n", __func__, external_authenticate_support);
	if (use_more_precise_NTC_table) {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (!gm->disableGM30 || is_recovery_mode(gm)) {
