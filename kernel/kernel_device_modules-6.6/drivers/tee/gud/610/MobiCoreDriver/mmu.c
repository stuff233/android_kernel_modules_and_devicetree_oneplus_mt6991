			     ...
			     struct page **pages)
{
	...
	ret = pin_user_pages(start, nr_pages, gup_flags, pages);

	if (ret == -ENOMEM){
		int i;
		pr_err("gup_local: task=%s, start=%lx nr_pages=%ld\n",
				current->comm, start, nr_pages);
		nr_pages = get_user_pages(start, nr_pages, 0, pages);
		if (nr_pages > 0)
			for (i = 0; i < nr_pages; i++){
				pr_err("pa=%llu type=%lx cnt=%d\n", page_to_phys(pages[i]), get_pageblock_migratetype(pages[i]), page_count(pages[i]));
				put_page(pages[i]);
			}
		else
			pr_err("get_user_pages ret=%ld\n", nr_pages);
	}
