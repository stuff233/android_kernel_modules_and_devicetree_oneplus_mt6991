...
#ifdef OPLUS_FEATURE_DISPLAY
		pr_err(" us=%d, vidle[0]=%d,vidle[1]=%d,vidle[2]=%d,\n",us,g_vidle_support[0][0],g_vidle_support[0][1],g_vidle_support[0][2]);
			if ((us == 16666) && (g_vidle_support[0][0] !=0)) {
				pr_err("vidle 16666 60hz psz:%d\n",g_vidle_support[1][0]);
				dpc_dt_set_update(7,g_vidle_support[1][0]);
			} else if ((us == 11111) && (g_vidle_support[0][1] !=0) ){
				pr_err("vidle 1111 90hz psz %d\n",g_vidle_support[1][1]);
				dpc_dt_set_update(7,g_vidle_support[1][1]);
			} else if ((us == 8333) && (g_vidle_support[0][2] !=0)) {
				pr_err("vidle 8333 120hz psz%d\n",g_vidle_support[1][2]);
				dpc_dt_set_update(7,g_vidle_support[1][2]);
			} else {
