	...
	unsigned int panel_ext_param, unsigned int cfg_flag, unsigned int lock)
{
	...
	if ((cfg_flag & (0x1<<SET_BACKLIGHT_LEVEL)) && !(cfg_flag & (0x1<<SET_ELVSS_PN))
		...
		DDPINFO("%s cfg_flag = %d,level=%d\n", __func__, cfg_flag, level);
		if (comp && comp->funcs && comp->funcs->io_cmd) {
			//oplus 6382 stops vdo at mtk_dsi_io_cmd
	...
	if (oplus_ofp_is_supported()) {
		oplus_ofp_lhbm_handle(crtc, false);
	}
#endif /* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
...
#endif
#ifdef OPLUS_FEATURE_DISPLAY
unsigned int g_vidle_support[2][3] = {0};
EXPORT_SYMBOL(g_vidle_support);
#endif
				      ...
				      struct drm_atomic_state *atomic_state)
{
	...
	if (crtc_id == 0 && mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_VIDLE_FULL_SCENARIO)) {
		...
		if (mtk_crtc_state->disp_mode_changed) {
			...
			DDPINFO("mtk_drm_crtc_atomic_begin is_support_vidle=%d\n",panel_ext->params->is_support_vidle);
			if (crtc->state->mode.hskew == OPLUS_ADFR || crtc->state->mode.hskew == OPLUS_MFR ||
				mtk_crtc_state->prop_val[CRTC_PROP_DOZE_ACTIVE] || (panel_ext->params->is_support_vidle != true)) {
				DDPINFO("%s oa/aod/no support vidle serials close vidle\n", __func__);
				mtk_vidle_hint_update(OPLUS_CLOSE_VIDLE);
			} else {
				DDPINFO("%s open vidle\n", __func__);
				mtk_vidle_hint_update(OPLUS_OPEN_VIDLE);
			}
#endif /* OPLUS_FEATURE_DISPLAY */
		} else
	...
	if (!(oplus_apollo_unsupported())) {
		if(mtk_crtc_is_frame_trigger_mode(crtc)) {
			...
			oplus_sync_panel_brightness_video(crtc, mtk_crtc_state->cmdq_handle);
		}
	}
#endif /* OPLUS_FEATURE_DISPLAY_APOLLO */
	if ((priv->usage[crtc_idx] == DISP_OPENING) &&
				      ...
				      struct drm_atomic_state *atomic_state)
{
	...
	if (oplus_ofp_is_supported() && (index == 0)) {
		...
		oplus_ofp_lhbm_handle(crtc, false);
		oplus_ofp_aod_off_backlight_recovery(crtc, mtk_crtc_state);
	}
#else
...
#ifdef OPLUS_FEATURE_DISPLAY
		struct mtk_panel_ext *panel_ext = mtk_crtc->panel_ext;
		DDPINFO("mtk_drm_mode_switch_thread is_support_vidle=%d\n",panel_ext->params->is_support_vidle);

		if (crtc->state->mode.hskew == OPLUS_ADFR || crtc->state->mode.hskew == OPLUS_MFR || (panel_ext->params->is_support_vidle != true)) {
			DDPINFO("oa/no support vidle serials close vidle\n");
			mtk_vidle_config_ff(false);
		} else {
			DDPINFO("mode_switch close vidle follow dx4\n");
