...
#ifdef OPLUS_FEATURE_DISPLAY
				(((fp_type & 0xFF) != 0x10) && ( mtk_ddp_comp_get_type(comp->id) == MTK_DISP_CCORR))||
#else
...
#ifdef OPLUS_FEATURE_DISPLAY
		if (((fp_type & 0xFF) != 0x10) && (comp->doze_bypass & DOZE_BYPASS_PQ)) {
			if (mtk_crtc && mtk_crtc->panel_ext && mtk_crtc->panel_ext->params &&
							mtk_crtc->panel_ext->params->oplus_display_lcd_tp_aod == 1) {
				/* lcd aod doesn't bypass pq */
				/* mtk_ddp_comp_bypass(comp, bypass, PQ_FEATURE_KRN_DOZE, cmdq_handle); */
...
void mtk_drm_top_clk_prepare_enable(struct drm_crtc *crtc)
{
	...
	DDPINFO("mtk_drm_top_clk_prepare_enable is_support_vidle=%d\n",panel_ext->params->is_support_vidle);
	if (crtc->state->mode.hskew == OPLUS_ADFR || crtc->state->mode.hskew == OPLUS_MFR ||
		state->prop_val[CRTC_PROP_DOZE_ACTIVE] || (panel_ext->params->is_support_vidle != true)) {
...
#endif /* OPLUS_FEATURE_DISPLAY */
{
	...
	if (atomic_read(&top_clk_ref) == 0) {
		...
		#ifdef OPLUS_FEATURE_DISPLAY
			DDPINFO("%s vidle_hint(%#x)\n", __func__, mtk_vidle_hint_update(VIDLE_HINT_GET));
		#else /* OPLUS_FEATURE_DISPLAY */
			DDPINFO("%s crtc%d vidle_hint(%#x)\n", __func__, drm_crtc_index(crtc),
				mtk_vidle_hint_update(VIDLE_HINT_GET));
		#endif /* OPLUS_FEATURE_DISPLAY */
...
static void mtk_drm_kms_lateinit(struct kthread_work *work)
{
	...
	if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL)
	#ifdef OPLUS_FEATURE_DISPLAY
		mtk_drm_top_clk_disable_unprepare(private->crtc[0]->dev);
	#else /* OPLUS_FEATURE_DISPLAY */
