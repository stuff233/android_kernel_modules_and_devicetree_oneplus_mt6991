...
static void mtk_vidle_enable_v2(bool _en, void *_drm_priv)
{
	...
	if (_en && mtk_disp_vidle_flag.vidle_en)
...
void mtk_vidle_config_ff(bool _en)
{
	int ret = 0;
	bool en = false;
...
u32 mtk_vidle_hint_update(enum mtk_vidle_hint_type type)
{
	...
	case OPLUS_OPEN_VIDLE:
		vidle_data.hint.oplus_close_vidle = false;
		break;
	case OPLUS_CLOSE_VIDLE:
		vidle_data.hint.oplus_close_vidle = true;
		break;
...
#ifdef OPLUS_FEATURE_DISPLAY
		     vidle_data.hint.oplus_close_vidle |
#endif /* OPLUS_FEATURE_DISPLAY */
