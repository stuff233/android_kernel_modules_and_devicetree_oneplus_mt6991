...
CONFIG_REG:
	...
	if (dsi->ext && !dsi->ext->params->vdo_per_frame_lp_enable &&
		...
		dsi->hfp_minimum_dphy = DIV_ROUND_UP(2 + 4 + (da_hs_trail + 1 + (lpx + da_hs_exit +
				da_hs_prep + da_hs_zero + 2)) * dsi->lanes + 4, dsi_buf_bpp);
	else if (dsi->ext && !dsi->ext->params->vdo_per_frame_lp_enable &&
		(dsi->mode_flags & MIPI_DSI_CLOCK_NON_CONTINUOUS))
		dsi->hfp_minimum_dphy = DIV_ROUND_UP(2 + 4 + (da_hs_trail + 1 + clk_hs_post + 1 +
						clk_hs_trail + clk_hs_exit + 1 + lpx + clk_hs_prep +
						clk_hs_zero + (lpx + da_hs_exit + da_hs_prep +
						da_hs_zero + 2)) * dsi->lanes + 4, dsi_buf_bpp);
	else
...
irqreturn_t mtk_dsi_irq_status(int irq, void *dev_id)
{
	...
	if (status) {
		...
		if ((status & BUFFER_UNDERRUN_INT_FLAG)	&& (atomic_read(&mtk_crtc->force_high_step) == 0)) {
			...
			DDPPR_ERR("[dsi] underrun isr start\n");
...
static void mtk_output_en_doze_switch(struct mtk_dsi *dsi)
{
	...
	if (doze_enabled && panel_funcs->oplus_doze_enable) {
		...
		if ((!oplus_ofp_video_mode_30hz_aod_is_enabled() || (refresh_rate == 30)) && !oplus_ofp_video_mode_30hz_aod_accelerate_is_enabled()) {
			oplus_ofp_doze_status_handle(doze_enabled, dsi->encoder.crtc, dsi->ext, dsi->panel, dsi, mtk_dsi_cmdq_pack_gce);
		...
		} else if (oplus_ofp_video_mode_30hz_aod_accelerate_is_enabled()) {
			oplus_ofp_doze_status_handle(doze_enabled, dsi->encoder.crtc, dsi->ext, dsi->panel, dsi, mtk_dsi_cmdq_pack_gce);
			vdo_aod_cfg.aod_en = doze_enabled;
			mtk_dsi_vdo_aod_ctrl(dsi, mtk_crtc, &vdo_aod_cfg);
			oplus_ofp_set_aod_light_mode_after_doze_enable(dsi->ext, dsi, mtk_dsi_cmdq_pack_gce);
		} else if (panel_funcs->oplus_set_backlight_cmdq) {
			/*video mode 30hz aod is enabled,Avoid screen flickering caused by normal backlight*/
			panel_funcs->oplus_set_backlight_cmdq(dsi, NULL, NULL, 0);
			DDPINFO("video mode 30hz aod,%s doze_enabled set backlight 0\n", __func__);
		}
	}
#endif /* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	mtk_crtc->oplus_apollo_br->oplus_timing_switch_tag_ns = ktime_get();
#endif /* OPLUS_FEATURE_DISPLAY_APOLLO */
...
#ifdef OPLUS_FEATURE_DISPLAY
			DDPPR_ERR("%s:current cmdq size(%d) out of dsi cmdq size(128 bytes)\n", __func__, total_cmdq_size);
#else
		...
		if (!mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
			...
			if (ext->params->is_cphy) {
				...
				if (priv && (priv->data->mmsys_id == MMSYS_MT6899 ||
					...
					lpx = (lpx % 2) ? lpx + 1 : lpx;  //lpx must be even
					da_hs_prep = (dsi->data_rate * 50) / 7000 + 1;
					da_hs_prep = (da_hs_prep % 2) ? da_hs_prep + 1 : da_hs_prep;
					da_hs_zero = 48;
					da_hs_trail = 32;
					da_hs_exit = (dsi->data_rate * 118) / 7000 + 1;
					da_hs_exit = (da_hs_exit % 2) ? da_hs_exit + 2 : da_hs_exit + 1;
					...
					data_phy_cycle = da_hs_prep + da_hs_zero + da_hs_exit + 1 +
								lpx + cphy_progseq_cycle + 1;
				} else {
					ui = (1000 / data_rate > 0) ? 1000 / data_rate : 1;
					cycle_time = 7000 / data_rate;
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
...
#ifdef OPLUS_FEATURE_DISPLAY
	if (dsi->ext && dsi->ext->funcs && dsi->ext->funcs->mode_switch_update_for_vdo) {
		dsi->ext->funcs->mode_switch_update_for_vdo(&dsi->conn, src_mode, dst_mode);
	}
#endif /* OPLUS_FEATURE_DISPLAY */
	...
	if (fps_chg_index & MODE_DSI_CLK) {
		...
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DSI_TIME_CON1(dsi->driver_data), 0x80000000, 0x80000000);
#endif
		mtk_dsi_trigger(comp, handle);
		/*set ESD_EOF
			...
			struct mtk_drm_crtc *mtk_crtc, struct mtk_vdo_aod_cfg *vdo_aod_cfg)
{
	...
	if (dsi->ext && dsi->ext->funcs && dsi->ext->funcs->get_vdo_aod_param)
		dsi->ext->funcs->get_vdo_aod_param(vdo_aod_cfg->aod_en, m, dsi->panel, &vdo_aod_param);
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_WHITE_POINT_COMPENSATION:
	{
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if (panel_ext && panel_ext->funcs && panel_ext->funcs->lcm_set_white_point_compensation) {
			panel_ext->funcs->lcm_set_white_point_compensation(dsi, mtk_dsi_cmdq_pack_gce,
					handle, *(unsigned int *)params);
		}
		break;
	}
