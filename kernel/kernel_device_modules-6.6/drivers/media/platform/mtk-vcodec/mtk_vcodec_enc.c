...
static int vidioc_venc_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MPEG_MTK_ENCODE_VISUAL_QUALITY:
#ifdef MTK_HARDWARE_ENCODE_OPTIMIZATION
		mtk_v4l2_debug(2,
			"V4L2_CID_MPEG_MTK_ENCODE_VISUAL_QUALITY: quant(%d), rd(%d), pfrm_quant(%d), bfrm_quant(%d)",
		...
		mtk_v4l2_debug(2,
			...
			ctrl->p_new.p_s32[0], ctrl->p_new.p_s32[1]);
#endif /*MTK_HARDWARE_ENCODE_OPTIMIZATION*/
...
#if IS_ENABLED(CONFIG_MTK_TINYSYS_VCP_SUPPORT)
			if (ctx->q_data[MTK_Q_DATA_DST].fmt->fourcc != V4L2_PIX_FMT_HEIF &&
			    !ctx->enc_params.svp_mode && ctx->dev->support_acp) {
...
#ifdef MTK_HARDWARE_ENCODE_OPTIMIZATION
		mtk_v4l2_err("[%d] idx=%d, quant=%d, rd=%d, pfrm_quant=%d, bfrm_quant=%d, aqoffsetreduction=%d, lumaaqenable=%d",
				ctx->id,
				mtk_buf->vb.vb2_buf.index,
				...
				enc_prm.visual_quality->lumaaqenable);
#else
...
int mtk_vcodec_enc_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
	...
	ctx->enc_params.visual_quality.lumaaqenable = -1;
#endif /*MTK_HARDWARE_ENCODE_OPTIMIZATION*/
